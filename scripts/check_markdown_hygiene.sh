...
# documentation debt stays visible to the audit without blocking unrelated work.
REPO_ROOT="$(git -C "$(dirname "${BASH_SOURCE[0]}")" rev-parse --show-toplevel)"
IGNORED_BASELINE="$REPO_ROOT/scripts/markdown_hygiene_ignored_baseline.txt"
BASE_REF=""
MAX_NEW="${POKEMAP_MARKDOWN_MAX_NEW:-0}"
MAX_BYTES="${POKEMAP_MARKDOWN_MAX_BYTES:-262144}"

usage() {
  echo "Usage: bash scripts/check_markdown_hygiene.sh [--base <git-ref>]" >&2
}
...
# Ignored Markdown is still local clutter. The baseline records the existing
# ignored debt so this guard can reject only newly introduced files. Disposable
# dependency/build trees are excluded because their contents are tool-owned.
while IFS= read -r path; do
  case "$path" in
  ...
  if [[ -f "$IGNORED_BASELINE" ]] && grep -Fqx -- "$path" "$IGNORED_BASELINE"; then
    continue
  fi
  printf '%s\n' "$path" >> "$candidate_file"
  printf '%s\n' "$path" >> "$new_file"
...
while IFS= read -r path; do
  if is_markdown_path "$path"; then
    echo "Ignored Markdown is not allowed outside the audited baseline: $path" >&2
    errors=1
  fi
done < "$ignored_file"
