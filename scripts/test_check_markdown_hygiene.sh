...
CHECKER="$REPO_ROOT/scripts/check_markdown_hygiene.sh"
TEST_ROOT="$(mktemp -d)"
...
new_fixture() {
  ...
  cp "$CHECKER" "$fixture/scripts/check_markdown_hygiene.sh"
  ...
  git -C "$fixture" add legacy.md scripts/check_markdown_hygiene.sh
...
expect_pass \
  ...
  bash "$FIXTURE/scripts/check_markdown_hygiene.sh"
...
mkdir -p "$FIXTURE/reports/analysis"
printf '# Initially canonical\n' > "$FIXTURE/reports/analysis/allowed.md"
git -C "$FIXTURE" add reports/analysis/allowed.md
git -C "$FIXTURE" commit -qm "add canonical report"
mkdir -p "$FIXTURE/misc"
mv "$FIXTURE/reports/analysis/allowed.md" "$FIXTURE/misc/moved.md"
git -C "$FIXTURE" add -A
expect_fail_with \
...
mkdir -p "$FIXTURE/reports/analysis"
mv "$FIXTURE/legacy.md" "$FIXTURE/reports/analysis/legacy.md"
expect_pass \
...
mkdir -p "$FIXTURE/reports/analysis"
cp "$FIXTURE/legacy.md" "$FIXTURE/reports/analysis/first-copy.md"
cp "$FIXTURE/legacy.md" "$FIXTURE/reports/analysis/second-copy.md"
...
printf '# Orphan report\n' > "$FIXTURE/reports/orphan.md"
expect_fail_with \
  "reports require a domain directory" \
  "reports/orphan.md" \
  env POKEMAP_MARKDOWN_MAX_NEW=1 \
  bash "$FIXTURE/scripts/check_markdown_hygiene.sh"

new_fixture orphan_doc
mkdir -p "$FIXTURE/docs"
printf '# Orphan doc\n' > "$FIXTURE/docs/orphan.md"
expect_fail_with \
  "durable docs require a domain directory" \
  "docs/orphan.md" \
  env POKEMAP_MARKDOWN_MAX_NEW=1 \
  bash "$FIXTURE/scripts/check_markdown_hygiene.sh"
...
printf '# Duplicate source dump\n' > \
  "$FIXTURE/reports/analysis/lot_created_files_full_content.md"
expect_fail_with \
...
head -c 262145 /dev/zero | tr '\0' 'x' > \
  "$FIXTURE/reports/analysis/oversized.md"
expect_fail_with \
...
mkdir -p "$FIXTURE/reports/analysis"
printf '# First\n' > "$FIXTURE/reports/analysis/first.md"
printf '# Second\n' > "$FIXTURE/reports/analysis/second.md"
expect_fail_with \
...
echo "PASS: 18 Markdown hygiene scenarios"
