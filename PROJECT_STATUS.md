...
| Modèles scénario (`ScenarioAsset`, `ScenarioNode`, `ScenarioEdge`) | **Fait (LOT 54)** | Persistés dans `ProjectManifest.scenarios` |
...
| Opérations pures sur les données (19+ modules) | **Fait** | Resize, paint, collision, terrain, path, layers, entities, warps, triggers, zones, connections, metadata, `map_placed_elements`, `map_events`, tileset/dialogue library trees |
...
| Event authoring panel (MVP) | **Fait** | Création/suppression/sélection d’events, marqueurs canvas, édition id/title/type/layer/position/pages, script + startNode via dropdown, condition MVP simple (`none`/`flag set`/`flag unset`/`event consumed`/JSON brut) |
| Scenario workspace central (MVP) | **Fait (LOT 54)** | Mode `scenario` dans le centre (à la place de la map), canvas de graphe pannable/zoomable, nœuds typés, connexions visibles, sélection et déplacement |
| Scenario library panel | **Fait (LOT 54)** | Section `Scenario Graphs` dans l’explorer gauche : create/rename/delete/select |
| Scenario inspector panel | **Fait (LOT 54)** | Inspecteur droit : édition scenario + nœud (type/titre/description/bindings/payload) et gestion des edges |
...
| Project explorer panel | **Fait** | maps, groupes, tilesets, éléments, dialogues, scénarios, scripts, dresseurs, personnages |
