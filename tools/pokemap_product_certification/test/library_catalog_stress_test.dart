...
void main() {
  test(
    'library round-trips 100 independent games inside the reference guard',
    () async {
      final root = await Directory.systemTemp.createTemp(
        'pokemap-phase8-library-',
      );
      addTearDown(() async {
      ...
      expect(
        reopened.library.games.first.gameId,
        'games.certification.game000',
      );
      expect(reopened.library.games.last.gameId, 'games.certification.game099');
      expect(
        measurement.passed,
        isTrue,
        reason: measurement.toJson().toString(),
      );
    },
  );

  test(
    'export and hostile reopen cover a 2000-entry species catalogue',
    () async {
      final root = await Directory.systemTemp.createTemp(
        'pokemap-phase8-catalog-',
      );
      addTearDown(() async {
        if (await root.exists()) await root.delete(recursive: true);
      });
      ...
      final artifact = await fixture.export(
        authorRoot,
        File(p.join(root.path, 'catalog.avelunegame')),
      );
      watch.stop();
      final measurement = ProductCertificationMeasurement.evaluate(
      ...
      expect(
        artifact.manifest.content.files.where(
          (entry) => entry.path.contains('/species/'),
        ),
