...
void main() {
  ...
  test('second neutral game exports, loses its source and installs offline',
      () async {
    final root =
        await Directory.systemTemp.createTemp('pokemap-phase8-neutral-');
    addTearDown(() async {
    ...
    final packageFile = File(p.join(root.path, 'neutral-1.0.0.pokemapgame'));
    ...
    await for (final entity
        in installedVersion.list(recursive: true, followLinks: false)) {
      if (entity is! File ||
          !const <String>{'.json', '.txt'}.contains(p.extension(entity.path))) {
        continue;
      }
      expect(
