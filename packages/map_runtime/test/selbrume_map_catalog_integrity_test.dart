...
void _validateLegacyMapFiles({
  ...
  final activeLegacyPaths = referencedAbsolutePaths
      .map(p.normalize)
      .toSet()
      .intersection(expectedLegacyPaths);
  ...
  for (final fileName in expectedLegacyFileNames) {
    final expectedFile = File(p.join(mapsDirectory.absolute.path, fileName));
    if (!expectedFile.existsSync()) {
      throw StateError(
          'Expected legacy map file is missing: ${expectedFile.path}');
    }
  }
  ...
  for (final file in mapJsonFiles) {
    ...
    if (referencedAbsolutePaths.contains(absolutePath)) {
      continue;
    }
    final expectedLegacyPath = p.normalize(
      p.join(mapsDirectory.absolute.path, p.basename(absolutePath)),
    );
    if (absolutePath != expectedLegacyPath) {
      throw StateError(
        'Legacy map JSON files must remain directly under maps/: $absolutePath',
      );
    }
    if (!expectedLegacyFileNames.contains(p.basename(absolutePath))) {
