...
void main() {
  group('Trainer defeated state', () {
    test('debugMarkTrainerAsDefeated poses flag in storyFlags', () {
      ...
      final updatedState = initialState.copyWith(
        storyFlags: initialState.storyFlags.copyWith(
          activeFlags: {
            ...initialState.storyFlags.activeFlags,
            'trainer_defeated:trainer_001'
          },
        ),
      );

      expect(updatedState.storyFlags.activeFlags,
          contains('trainer_defeated:trainer_001'));
    });
    ...
    test('trainer defeated flag uses convention trainer_defeated:{trainerId}',
      ...
      const expectedFlag = 'trainer_defeated:$trainerId';
    ...
    test('multiple trainers can be marked defeated independently', () {
      ...
      final state2 = state1.copyWith(
        storyFlags: state1.storyFlags.copyWith(
          activeFlags: {
            ...state1.storyFlags.activeFlags,
            'trainer_defeated:trainer_002'
          },
        ),
      );

      expect(state2.storyFlags.activeFlags,
          contains('trainer_defeated:trainer_001'));
      expect(state2.storyFlags.activeFlags,
          contains('trainer_defeated:trainer_002'));
    ...
    test('defeatDialogueRef is present in MapEntityNpcData model', () {
      ...
      expect(entity.npc?.defeatDialogueRef?.dialogueId,
          equals('defeat_dialogue_1'));
    });
  ...
  group('Trainer interaction fallback chain', () {
    test('trainer battu + defeatDialogueRef => defeat dialogue path', () {
      ...
      const entityWithDefeat = MapEntity(
        id: 'npc_1',
    ...
    test(
        'trainer battu + pas de defeatDialogueRef + dialogue => fallback dialogue',
        () {
      const entityWithNormal = MapEntity(
        id: 'npc_2',
      ...
      final shouldFallbackToNormal =
          defeatRef == null && normalDialogue != null;

      expect(shouldFallbackToNormal, isTrue);
    });

    test('trainer battu + aucun dialogue => notification path', () {
      const entityNoDialogue = MapEntity(
        id: 'npc_3',
