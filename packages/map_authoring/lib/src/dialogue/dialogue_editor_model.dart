        ...
        branches: branches
            .map(
              (branch) => DeChoiceBranch(
                ...
                steps: branch.steps
                    .map((inner) =>
                        _cloneDialogueStep(inner, freshIds: freshIds))
                    .toList(),
              ),
            )
            .toList(),
...
DialogueEditorDocument cloneDialogueDocument(DialogueEditorDocument document) =>
    DialogueEditorDocument(
      nodes: document.nodes.map(_cloneDialogueNode).toList(),
      entryNodeId: document.entryNodeId,
      sourcePreservation: document.sourcePreservation,
    );

DialogueEditorStep cloneDialogueStep(DialogueEditorStep step,
        {bool freshIds = false}) =>
    _cloneDialogueStep(step, freshIds: freshIds);
