  ...
  for (final raw in content.split('\n')) {
    ...
    if (!inBody) {
      ...
      if (trimmed == '===') {
        ...
        currentChoiceText = _decodeText(trimmed.substring(2).trim());
      } else if (trimmed.startsWith('<<jump ') && trimmed.endsWith('>>')) {
        if (inChoiceBlock) closeChoiceBlock();
        final target =
            trimmed.substring('<<jump '.length, trimmed.length - 2).trim();
...
DialogueEditorStep _lineStepFromPlainText(
  ...
  trimmed = _decodeText(trimmed);
  // Narration : convention éditeur `(didascalie)` sur une seule ligne.
  final nar = RegExp(r'^\((.*)\)\s*$', dotAll: true).firstMatch(trimmed);
  if (nar != null) {
...
List<DialogueEditorStep> _convertParsedSteps(List<_PStep> steps) {
  ...
  for (var index = 0; index < steps.length; index++) {
    ...
    if (step case _PRaw(:final line)) {
      ...
      final speaker = _speakerDirective.firstMatch(line.trim());
      ...
      if ((portrait != null || speaker != null) &&
        ...
        converted.add(
          _lineStepFromPlainText(
            next.text,
            characterId: portrait?.group(1) ?? speaker?.group(1),
...
final RegExp _speakerDirective = RegExp(r'^<<speaker\s+([^\s>]+)>>$');
  ...
  switch (step) {
    case DeLineStep(
      ...
      if (characterId != null && portraitStateId != null) {
        ...
        sb.writeln('$indent<<speaker ${characterId.trim()}>>');
      }
      final sp = speaker;
      final line =
          (sp != null && sp.trim().isNotEmpty) ? '${sp.trim()}: $body' : body;
      sb.writeln('$indent${_encodeText(line)}');
    case DeNarrationStep(:final text):
      sb.writeln('$indent${_encodeText('($text)')}');
    ...
    case DeChoiceStep(:final branches):
      for (final b in branches) {
        sb.writeln('$indent-> ${_encodeText(b.label)}');
...
String _decodeText(String value) {
  ...
  final decoded = jsonDecode(value.substring(1));
  if (decoded is! String) {
    throw const FormatException('Invalid literal dialogue text.');
  }
  return decoded;
}

String _encodeText(String value) => value.trim() != value ||
        value.contains('\n') ||
        value.contains('\r') ||
        value.contains('<<') ||
        value.startsWith('->') ||
