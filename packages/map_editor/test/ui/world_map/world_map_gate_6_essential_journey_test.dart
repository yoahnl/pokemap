...
void main() {
  for (final size in <Size>[const Size(800, 600), const Size(1280, 800)]) {
    testWidgets('Gate 6 essential journey stays one-click and transactional at '
      ...
      final selectedManualCell = _cellCenter(tester, const GridPos(x: 4, y: 2));
      ...
      expect(_manualTree(editor.state).pos, const GridPos(x: 5, y: 2));
...
Future<int> _activateLayerIfNeeded(WidgetTester tester, String layerId) async {
  ...
  Finder active() =>
      find.byKey(ValueKey<String>('world-map-layer-active-$layerId'));
  Finder activation() =>
      find.byKey(ValueKey<String>('world-map-layer-activate-$layerId'));
...
Future<void> _activateFamily(
  ...
  await tester.tap(find.byKey(ValueKey<String>('world-map-tool-$family')));
  await tester.pumpAndSettle();
  expect(
...
bool _collisionAt(EditorState state, int x, int y) {
  final layer = state.activeMap!.layers.whereType<CollisionLayer>().singleWhere(
    (candidate) => candidate.id == 'collision',
  );
...
MapPlacedElement _manualTree(EditorState state) => state
    .activeMap!
    .placedElements
    .singleWhere((entry) => entry.id == 'manual-tree');

Future<_JourneyHarness> _pumpJourney(WidgetTester tester, Size size) async {
    ...
    : container = ProviderContainer(
        overrides: <Override>[
          editorImageCacheProvider.overrideWith(
            (ref, projectRoot) =>
                _ImmediateEditorImageCache(projectRoot, image),
          ),
        ],
      ),
      _image = image {
