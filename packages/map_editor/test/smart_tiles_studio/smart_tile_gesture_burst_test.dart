...
void main() {
  ...
  // the editor visibly flickered: adopting the snapshot of the burst that just
  // landed wiped the cells clicked since, until their own write put them back.
  // These two watch every intermediate state instead.
  test('a burst never blinks a painted cell off the canvas', () async {
    final h = await _Harness.create();
    addTearDown(h.dispose);
    h.observe();
    ...
    var seen = <GridPos>{};
    for (final frame in h.frames) {
      expect(
        seen.difference(frame),
        isEmpty,
        reason: 'these cells left the canvas after being painted: '
            '${seen.difference(frame)}',
      );
      seen = seen.union(frame);
    }
    expect(seen, hasLength(3), reason: 'no click may be dropped');
  });

  test('a burst never blinks an erased cell back onto the canvas', () async {
    final h = await _Harness.create();
    addTearDown(h.dispose);
    for (var x = 0; x < 3; x++) {
      h.paint(GridPos(x: x, y: 0));
    ...
    for (final frame in h.frames) {
      expect(
        gone.intersection(frame),
        isEmpty,
        reason: 'these cells came back after being erased: '
            '${gone.intersection(frame)}',
      );
      gone = gone.union(<GridPos>{
        for (var x = 0; x < 3; x++) GridPos(x: x, y: 0),
      }.difference(frame));
    }
...
final class _Harness {
  ...
  final List<Set<GridPos>> frames = <Set<GridPos>>[];
  ...
  /// Samples the canvas while [settle] waits, at roughly rendering cadence.
  ///
  /// Sampling rather than listening is deliberate: a flicker the author can see
  /// lasts as long as a canonical write, and a change reverted within the same
  /// microtask never reaches the screen at all.
  void observe() {
    _observing = true;
    frames.add(_paintedIn(notifier.state));
  }
  ...
  Future<void> settle() async {
    for (var attempt = 0; attempt < 400; attempt++) {
      ...
      if (_observing) frames.add(_paintedIn(notifier.state));
  ...
  List<GridPos> painted() => _paintedIn(notifier.state).toList();

  static Set<GridPos> _paintedIn(EditorState state) {
