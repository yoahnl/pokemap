...
void main() {
  testWidgets(
    'previews every player surface and dispatches guided token edits',
    (tester) async {
      ...
      expect(find.text('HUD exploration'), findsNothing);
      expect(find.text('Combat'), findsOneWidget);

      final titleSurfaceButton = find.byKey(
        const ValueKey<String>('theme-edit-titleSurface'),
      );
  ...
  testWidgets('blocks publication feedback and offers the safe fallback', (
    ...
    await tester.tap(find.byKey(const ValueKey<String>('theme-safe-fallback')));
