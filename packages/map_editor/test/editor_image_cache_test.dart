...
void main() {
  ...
  test(
    'reuses decoded pixels through distinct consumer image handles',
    () async {
  ...
  test('reports production image cache filesystem work to telemetry', () async {
    final file = File('${tempDir.path}/tiles.png');
    await file.writeAsBytes(_png(width: 1, height: 1), flush: true);
  ...
  test(
    'invalidates a same-path image when the file fingerprint changes',
    () async {
      final file = File('${tempDir.path}/tiles.png');
      await file.writeAsBytes(_png(width: 1, height: 1), flush: true);
      final cache = _immediateCache(tempDir);
  ...
  test(
    'consumer release is idempotent and independent from cache disposal',
    () async {
      final file = File('${tempDir.path}/tiles.png');
      await file.writeAsBytes(_png(width: 1, height: 1), flush: true);
      final cache = _immediateCache(tempDir);
  ...
  test('keeps decode variants isolated in the cache key', () async {
    ...
    expect(transformedAgain.image!.isCloneOf(transformed.image!), isTrue);
  ...
  test(
    'returns a typed decode diagnostic and recovers after replacement',
    () async {
  ...
  test(
    'evicts the least-recently-used master by decoded byte weight',
    () async {
      ...
      final cache = _boundedImmediateCache(tempDir, maximumDecodedBytes: 8);
  ...
  test('keeps an acquired lease usable after its master is evicted', () async {
    ...
    final cache = _boundedImmediateCache(tempDir, maximumDecodedBytes: 4);
  ...
  test(
    'serves an oversized image without retaining its decoded master',
    () async {
      final file = File('${tempDir.path}/oversized.png');
      await file.writeAsBytes(_png(width: 3, height: 3), flush: true);
      final cache = _boundedImmediateCache(tempDir, maximumDecodedBytes: 16);
      var decodeCount = 0;
  ...
  test('shares one in-flight decode across concurrent loads', () async {
    ...
    final cache = _boundedImmediateCache(tempDir, maximumDecodedBytes: 64);
  ...
  test(
    'autoDispose releases a cache when its provider listener closes',
    () async {
      final container = ProviderContainer();
  ...
  test(
    'invalidating a provider replaces and disposes its project owner',
    () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final subscription = container.listen(
