...
void main() {
  ...
  testWidgets(
    'Items catalog shows empty state when project has no local items',
    (tester) async {
      await _pumpItemsWorkspace(
        tester,
        initialState: const EditorState(
  ...
  testWidgets('Items catalog lists local items and selects the first item', (
    ...
    expect(
      find.byKey(const Key('items-catalog-detail-poke-ball')),
      findsOneWidget,
    );
  ...
  testWidgets(
    'Items catalog keeps valid items visible when diagnostics exist',
    (tester) async {
      await _pumpItemsWorkspace(
        tester,
        initialState: const EditorState(
            ...
            (_) async => const PokemonItemsCatalogView(
              ...
              diagnostics: <PokemonItemsCatalogDiagnostic>[
                PokemonItemsCatalogDiagnostic(
                  message:
                      'Items catalog entry "broken-item" has an empty name.',
                  entryId: 'broken-item',
                  entryIndex: 1,
                ),
              ],
  ...
  testWidgets('Items catalog detail formats missing values as dash', (
    ...
    expect(
      find.byKey(const Key('items-catalog-detail-mystery-item')),
      findsOneWidget,
    );
  ...
  test(
    'Items catalog local sprite helper detects an existing local sprite',
    () async {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('items-catalog-local-sprite-preview')),
        findsNothing,
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await _pumpItemsWorkspace(
        ...
        overrides: [
          pokemonItemsCatalogWorkspaceLoaderProvider.overrideWithValue(
            (_) async => const PokemonItemsCatalogView(
              entries: <PokemonItemCatalogEntryView>[
                PokemonItemCatalogEntryView(id: 'poke-ball', name: 'Poké Ball'),
              ],
              isAvailable: true,
              description: 'Catalogue local des objets.',
            ),
      ...
      await tester.tap(
        find.byKey(const Key('items-catalog-preview-sync-button')),
      );
      await tester.pump();
