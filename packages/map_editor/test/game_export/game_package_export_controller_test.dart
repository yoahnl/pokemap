...
void main() {
  test(
    'loads a draft, exports and queues the same certified package',
    () async {
      ...
      final output = File(p.join(root.parent.path, 'controller.avelunegame'));
      addTearDown(() async {
      ...
      expect(controller.snapshot.status, GamePackageExportStatus.succeeded);
      expect(
        controller.snapshot.installRequest?.requestId,
  ...
  test(
    'reports safe validation errors and returns to an editable state',
    () async {
      ...
      await controller.exportDraft(
        ...
        File(p.join(root.path, 'invalid.avelunegame')),
      );
  ...
  test(
    'exposes creator gameplay diagnostics when publication is blocked',
    () async {
      final root = await createAuthorProject(withDialogue: false);
      addTearDown(() => root.delete(recursive: true));
      ...
      addTearDown(controller.dispose);
      await controller.initialize();
      final output = File(p.join(root.parent.path, 'unplayable.avelunegame'));
      addTearDown(() async {
        if (await output.exists()) await output.delete();
      });
  ...
  test('names the missing project file and explains how to fix it', () async {
    ...
    final output = File(p.join(root.parent.path, 'missing-file.avelunegame'));
    addTearDown(() async {
      if (await output.exists()) await output.delete();
    });
    await controller.export(
    ...
    expect(controller.snapshot.safeErrorMessage, contains('dossier du projet'));
    expect(
      controller.snapshot.safeErrorMessage,
  ...
  test(
    'translates the manifest quota into an actionable player message',
    () async {
      final root = await createAuthorProject();
      addTearDown(() => root.delete(recursive: true));
      final controller = GamePackageExportController(
      ...
      final output = File(p.join(root.parent.path, 'oversized.avelunegame'));
      addTearDown(() async {
        if (await output.exists()) await output.delete();
      });
      await controller.export(
      ...
      expect(controller.snapshot.safeErrorMessage, contains('4 Mio'));
      expect(
        controller.snapshot.safeErrorMessage,
  ...
  test(
    'names the JSON file containing a reference outside the package',
    () async {
      final root = await createAuthorProject(withDialogue: false);
      addTearDown(() => root.delete(recursive: true));
      await File(
        p.join(root.path, 'data', 'escaping-reference.json'),
      ).writeAsString('{"assetPath":"../../outside.png"}', flush: true);
      final controller = GamePackageExportController(
        projectRoot: root,
        projectName: 'Neutral Adventure',
        profileStore: GamePackageExportProfileStore(projectRoot: root),
      ...
      addTearDown(controller.dispose);
      await controller.initialize();
      final output = File(p.join(root.parent.path, 'escaping.avelunegame'));
      addTearDown(() async {
        if (await output.exists()) await output.delete();
      });
  ...
  test(
    'persists and exposes the exact filesystem failure diagnostic',
    () async {
      final root = await createAuthorProject(withDialogue: false);
      final outputDirectory = Directory(
        p.join(root.parent.path, 'blocked-output.avelunegame'),
      );
      ...
      expect(
        controller.snapshot.safeErrorMessage,
        contains('blocked-output.avelunegame'),
      );
      expect(
      ...
      expect(persistedDiagnostic, contains('blocked-output.avelunegame'));
    },
  );
}
