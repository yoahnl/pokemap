...
void main() {
  group('BorderAssetSnapshotService', () {
    ...
    test(
      'deduplicates identical pixels independently from the source path',
      () {
    ...
    test('matches the canonical authoring snapshot compiler byte-for-byte', () {
      final pixels = _png(<int>[255, 0, 0, 255, 0, 0, 0, 0]);
      final editor = service.prepare(
        BorderAssetSnapshotRequest(
          sourceElementId: 'shared-element',
          anchorPx: const BorderPixelPos(x: 0, y: 0),
          frames: <BorderAssetSnapshotSourceFrame>[
            BorderAssetSnapshotSourceFrame(
              sourceProjectRelativePath: 'assets/tilesets/shared.png',
              encodedImageBytes: pixels,
              durationMs: 75,
            ),
          ],
      ...
      final canonical = const CanonicalBorderSnapshotCompiler().prepare(
        sourceElementId: 'shared-element',
        anchorPx: const BorderPixelPos(x: 0, y: 0),
      ...
      expect(canonical.snapshot, editor.snapshot);
      expect(canonical.metrics, editor.metrics);
      expect(
        canonical.files.map((file) => file.relativePath),
        editor.files.map((file) => file.relativePath),
      );
      expect(
        canonical.files.map((file) => file.bytes),
        editor.files.map((file) => file.bytes),
      );
    });

    test(
      'deduplicates identical RGBA pixels across different PNG encodings',
      () {
    ...
    test('freezes source bytes and exposes defensive file payload copies', () {
      ...
      expect(
        service.prepare(request).snapshot.contentFingerprint,
        frozenFingerprint,
      );
    ...
    test(
      'persists normalized animation metadata and aggregate alpha metrics',
      () {
        final first = _png(<int>[10, 20, 30, 255, 40, 50, 60, 255]);
        final second = _png(<int>[10, 20, 30, 0, 40, 50, 60, 255]);
        ...
        expect(prepared.snapshot.frames.map((frame) => frame.durationMs), <int>[
          100,
          75,
        ]);
        expect(
        ...
        BorderAssetSnapshotRequest(
          ...
          frames: <BorderAssetSnapshotSourceFrame>[
            BorderAssetSnapshotSourceFrame(
              ...
              encodedImageBytes: _png(<int>[1, 2, 3, 255, 4, 5, 6, 0]),
              durationMs: 100,
            ),
            BorderAssetSnapshotSourceFrame(
              sourceProjectRelativePath: 'assets/source/golden-b.png',
              encodedImageBytes: _png(<int>[7, 8, 9, 255, 10, 11, 12, 255]),
              durationMs: 75,
