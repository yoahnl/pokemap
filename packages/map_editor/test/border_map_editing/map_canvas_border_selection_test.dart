...
void main() {
  testWidgets(
    ...
    (tester) async {
        ...
        layers: <MapLayer>[
          MapLayer.border(
            ...
            content: BorderLayerContent(
              features: <BorderFeature>[lower, upper],
            ),
          ),
        ],
      );
      ...
      expect(
        ...
        isNull,
        reason:
            'objects painted above Border must win the common selection hit',
      );
      expect(
        container.read(editorNotifierProvider).activeMap!.toJson(),
        mapWithWarp.toJson(),
  ...
  testWidgets(
    ...
    (tester) async {
      final sourceStroke = BorderStroke(
        id: 'existing-wall',
        points: <GridPos>[for (var x = 0; x <= 4; x += 1) GridPos(x: x, y: 0)],
        closed: false,
      );
      final fixture = await _pumpLineCanvas(
        tester,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      for (final target in const <Offset>[Offset(144, 80), Offset(80, 144)]) {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await mouse.moveTo(fixture.canvas.topLeft + const Offset(32, 32));
      await tester.pump();
      expect(
      ...
      await gesture.moveTo(fixture.canvas.topLeft + const Offset(160, 0.5));
      await tester.pump();
      await gesture.moveTo(fixture.canvas.topLeft + const Offset(160, 160));
      await tester.pump();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      Future<void> drawHorizontalStroke(double y) async {
        ...
        await gesture.moveTo(fixture.canvas.topLeft + Offset(128, y));
        await tester.pump();
        await gesture.up();
        await tester.pump();
      }
      ...
      final firstPreview = fixture.container.read(
        borderPreviewControllerProvider,
      );
      ...
      final secondPreview = fixture.container.read(
        borderPreviewControllerProvider,
      );
      expect(secondPreview.phase, BorderPreviewPhase.resolved);
      expect(
        secondPreview.transaction!.baseFeatureFingerprint,
        firstPreview.transaction!.baseFeatureFingerprint,
      );
      expect(
        secondPreview.transaction!.variationOrdinal,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await firstGesture.moveTo(fixture.canvas.topLeft + const Offset(128, 32));
      await tester.pump();
      ...
      expect(
        restored.transaction!.baseFeatureFingerprint,
        firstTransaction.baseFeatureFingerprint,
      );
      expect(
        restored.transaction!.variationOrdinal,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await firstGesture.up();
      await tester.pump();
      final resolvedForA = fixture.container.read(
        borderPreviewControllerProvider,
      );
      ...
      final afterRejectedResume = fixture.container.read(
        borderPreviewControllerProvider,
      );
  ...
  testWidgets(
    'line erase creates an explicit opening without mutating the map',
    (tester) async {
      final sourceStroke = BorderStroke(
        id: 'wall',
        points: <GridPos>[for (var x = 0; x <= 4; x += 1) GridPos(x: x, y: 0)],
      ...
      expect(
        lineage.map((identity) => identity.preserveTraversal),
        everyElement(isTrue),
      );
      expect(
...
_pumpLineCanvas(
  ...
  final preview = createBorderPreviewControllerForOverride(
