...
void main() {
  test('ResizeMapUseCase blocks non-Border cell loss before mutation', () {
    const source = MapData(
      ...
      layers: <MapLayer>[
        MapLayer.tile(id: 'ground', name: 'Ground', cells: <int>[0, 9]),
      ],
    );
  ...
  test(
    'resizeActiveMap rejects Border errors without mutation or history',
    () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final notifier = container.read(editorNotifierProvider.notifier);
      ...
      container
          ...
          .setFeedback(_staleFeedback(malformed));
  ...
  test(
    'resizeActiveMap refuses to guess Border tile size without a project',
    () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final notifier = container.read(editorNotifierProvider.notifier);
      final source = _borderMap();
      notifier.state = EditorState(activeMap: source, activeLayerId: 'borders');
  ...
  test(
    ...
    () async {
      ...
      expect(notifier.planActiveMapResize(2, 1)?.canApply, isFalse);
    },
  );

  test(
    'resizeActiveMap treats same-size resize as a history-free no-op',
    () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final notifier = container.read(editorNotifierProvider.notifier);
      ...
      container
          .read(borderResizeFeedbackProvider.notifier)
          .setFeedback(_staleFeedback(source));
...
MapData _borderMap({int regionWidth = 3, bool destructiveEdge = true}) =>
    MapData(
      id: 'map',
