  ...
  group('WorldMapWorkspace', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.descendant(
            of: find.byKey(const ValueKey<String>('world-map-inspector-slot')),
        ...
        expect(
          tester
              .widget<Focus>(
                find.byKey(const ValueKey<String>('workspace-test-tool-focus')),
              )
              .focusNode,
          same(toolFocusNode),
        );
        ...
        expect(
          find.byKey(const ValueKey<String>('world-map-inspector-dock')),
          findsOneWidget,
        );
        expect(
          find.byKey(const ValueKey<String>('world-map-inspector-overlay')),
          findsNothing,
        );
    ...
    testWidgets('context erase commits one exact 1x1 transaction', (
        ...
        layers: <MapLayer>[
          sourceLayer.copyWith(
            palette: List<TileLayerPaletteEntry>.generate(
              7,
              (index) =>
                  TileLayerPaletteEntry(tilesetId: 'tiles', localTileId: index),
              growable: false,
            ),
      ...
      expect(resultLayer.cells.where((tile) => tile == 7), hasLength(63));
    ...
    testWidgets('move delegates through selection tool activation', (
      ...
      editorNotifier.state = editor.copyWith(
        activeMap: map,
        savedMapSnapshot: map,
      );
    ...
    testWidgets(
      'object deletion always crosses confirmation and commits once',
      (tester) async {
        var confirmed = false;
    ...
    testWidgets(
      'open target delegates the frozen typed intent without history',
      (tester) async {
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.byKey(const ValueKey<String>('project-explorer-reopen-toggle')),
          findsOneWidget,
        );
        expect(
          find.byKey(const ValueKey<String>('world-map-inspector-overlay')),
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.tap(
          find.byKey(const ValueKey<String>('workspace-test-inspector-toggle')),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          ...
          isA<WorldMapWorkspaceSession>()
              .having(
                (session) => session.explorerExpanded,
                'explorer',
                isFalse,
              )
              .having(
...
Future<ProviderContainer> _pumpWorkspace(
  ...
  if (seedRotationPreview) {
    final editor = container.read(editorNotifierProvider);
    container
        .read(mapPlacedElementRotationPreviewProvider.notifier)
        .preview(
          ...
          child: WorldMapWorkspace(
            ...
            explorerRailBuilder: (context, onReopen) {
              return PokeMapButton(
                key: const ValueKey<String>('project-explorer-reopen-toggle'),
                onPressed: onReopen,
...
EditorState _editorState() {
  final map = buildShellChromeMap(
    ...
    layers: const <MapLayer>[
      TileLayer(id: 'ground', name: 'Ground', cells: <int>[]),
    ],
  );
  return EditorState(
