...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const ValueKey('border-create-feature-button')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-feature-feature')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-rename-feature-button')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-move-feature-up-button')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-move-feature-down-button')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-delete-feature-button')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-layer-opacity-slider')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('border-layer-visibility-button')),
        findsOneWidget,
      );
  ...
  for (final template in <BorderBlueprintTemplate>[
    ...
    testWidgets(
      ...
      (tester) async {
        final project = _project(<BorderBlueprintRecord>[
          _record('cliff', name: 'Falaise', template: template),
        ]);
        final map = MapData(
          id: 'map',
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(preview.state.phase, BorderPreviewPhase.resolved);
      expect(
        preview.state.transaction!.proposedFeature.seed,
        isNot(initialSeed),
      );
      ...
      expect(
        container.read(editorNotifierProvider).activeMap!.name,
        'Aperçu appliqué',
      );
      expect(container.read(editorNotifierProvider).mapUndoStack, hasLength(1));
      expect(
        find.byKey(const ValueKey('border-feature-coast')),
        findsOneWidget,
      );
    },
  );
...
void _resolvePreview(
  ...
  preview.resolve(
    blueprintRevision: fixture.project.borderCatalog
        .recordById('coast-a')!
        .latestPublished!,
...
BorderPublishedPrimitive _stonePrimitive(String id, BorderPrimitiveRole role) =>
    BorderPublishedPrimitive(
      id: id,
      ...
      publishedMetrics: BorderPrimitiveAssetMetrics(
        ...
        occupancyMaskRle: encodeBorderRleMask(List<bool>.filled(12 * 24, true)),
      ),
    );
