...
void main() {
  test(
    'control center exposes guarded preflight and publish choices',
    () async {
      ...
      expect(workflow, isNot(contains('          - avelune')));
      expect(workflow, isNot(contains('          - both')));
      ...
      expect(workflow, isNot(contains('      avelune_version:')));
      expect(workflow, contains('confirmation:'));
    },
  );

  test(
    'control center validates requests before dispatching product workflows',
    () async {
      final workflow = await File(
        '../../.github/workflows/release_control_center.yml',
      ).readAsString();
      ...
      expect(validationJob, contains(r'test "$GITHUB_REF" = refs/heads/main'));
      expect(workflow, contains('environment: pokemap-release'));
      expect(workflow, contains('git ls-remote --exit-code --tags origin'));
      expect(workflow, contains(r'REQUEST_PRODUCT: ${{ inputs.product }}'));
      expect(workflow, contains(r'REQUEST_ACTION: ${{ inputs.action }}'));
      expect(
        workflow,
      ...
      expect(workflow, isNot(contains('avelune_android_release.yml')));
  ...
  test(
    'product workflows preserve tag releases and accept guarded dispatches',
    () async {
      final desktop = await File(
        '../../.github/workflows/pokemap_desktop_release.yml',
      ).readAsString();
      for (final workflow in <String>[desktop]) {
        ...
        expect(workflow, contains(r'test "$REQUEST_CONFIRMATION" = RELEASE'));
        expect(workflow, contains(r'test -n "$REQUESTED_VERSION"'));
        expect(
          workflow,
