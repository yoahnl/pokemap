...
void main() {
  ...
  PokemonDatabaseIndexEntry buildEntry({
    ...
    return PokemonDatabaseIndexEntry(
      ...
      refs: PokemonDatabaseIndexRefs(learnset: id, evolution: id, media: id),
  ...
  PokedexSpeciesDetail buildDetail({
    ...
    return PokedexSpeciesDetail(
      species: PokemonSpeciesFile(
        ...
        refs: PokemonSpeciesRefs(learnset: id, evolution: id, media: id),
  ...
  Future<void> selectPopupFilter(
    ...
    if (find.byKey(popupKey).evaluate().isEmpty) {
      final toggleFinder = find.byKey(
        const Key('pokedex-toggle-filters-button'),
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('pokedex-detail-empty-state')),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets('switches to forms learnset evolutions and media tabs', (
          ...
          evolution: const PokemonEvolutionFile(
            ...
            evolutions: [
              ...
              PokemonEvolutionEntry(
                targetSpeciesId: 'ivysaur',
                method: 'conditional',
                conditionText: {'fr': 'Monter de niveau sous la pluie'},
              ),
            ],
          ),
        ),
    ...
    expect(
      find.textContaining('Conservée au Pokédex — non exécutable en jeu.'),
      findsOneWidget,
    );
    expect(
      find.textContaining('Monter de niveau sous la pluie'),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.textContaining('Prévisualisation : 2 moves externes analysés.'),
        findsOneWidget,
      );

      await tester.tap(
        find.byKey(const Key('pokedex-moves-catalog-sync-button')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(find.byKey(const Key('pokedex-search-field')), '');
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const Key('pokedex-row-bulbasaur')));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('pokedex-generation-filter')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'shows a dedicated no results state when search matches nothing',
    (tester) async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      container.read(editorNotifierProvider.notifier).state = const EditorState(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final store = buildStore(
        details: <PokedexSpeciesDetail>[
          ...
          buildDetail(
            ...
            names: const <String, String>{'fr': 'Arcko', 'en': 'Treecko'},
          ),
        ],
      );
      ...
      await tester.ensureVisible(
        find.byKey(const Key('pokedex-edit-metadata-button')),
      );
      await tester.tap(find.byKey(const Key('pokedex-edit-metadata-button')));
      await tester.pumpAndSettle();

      await tester.ensureVisible(
        find.byKey(const Key('pokedex-enabled-switch')),
      );
      ...
      await tester.pumpAndSettle();
      await tester.ensureVisible(
        find.byKey(const Key('pokedex-save-metadata-button')),
      );
      ...
      expect(
        store.speciesById('bulbasaur').classification.isEnabledInProject,
        isFalse,
      );
      ...
      expect(store.speciesById('bulbasaur').typing.types, <String>[
        'electric',
        'fairy',
      ]);
      expect(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final store = buildStore(
        details: <PokedexSpeciesDetail>[
          ...
          buildDetail(
            id: 'ivysaur',
            nationalDex: 2,
            names: const <String, String>{'fr': 'Herbizarre', 'en': 'Ivysaur'},
          ),
        ],
      );
      ...
      expect(find.byKey(const Key('pokedex-detail-pane')), findsOneWidget);
      expect(
        find.byKey(const Key('pokedex-delete-species-button')),
        findsOneWidget,
      );
  ...
  testWidgets('imports a pokemon from the wizard and refreshes the workspace', (
      ...
      child: PokedexWorkspace(
        ...
        importer: (_, absoluteSpeciesSourcePath) async {
          ...
          final detail = buildDetail(
            ...
            names: const <String, String>{'fr': 'Pikachu', 'en': 'Pikachu'},
    ...
    await tester.tap(
      find.byKey(const Key('pokedex-empty-state-import-button')),
    );
    await tester.pumpAndSettle();
    ...
    expect(
      find.byKey(const Key('pokedex-import-preview-step')),
      findsOneWidget,
    );
    expect(
      find.byKey(const Key('pokedex-import-preview-title')),
      findsOneWidget,
    );
    ...
    await pumpPokedexWidget(
      ...
      child: PokedexWorkspace(
        ...
        externalImportPreviewer:
            (_, speciesQuery, {required mergePolicy}) async {
              previewCallCount += 1;
  ...
  testWidgets('cancel discards metadata changes without writing', (
    ...
    await tester.ensureVisible(
      find.byKey(const Key('pokedex-cancel-metadata-button')),
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const Key('pokedex-metadata-save-error')),
        findsOneWidget,
      );
      expect(
        find.text(
  ...
  testWidgets('edits forms and classification from the dedicated tab', (
    ...
    await tester.pumpAndSettle();
    await tester.ensureVisible(
      find.byKey(const Key('pokedex-edit-forms-button')),
    );
    ...
    await tester.pumpAndSettle();
    await tester.ensureVisible(
      find.byKey(const Key('pokedex-save-forms-button')),
    );
  ...
  testWidgets('creates a learnset locally from the dedicated tab', (
    ...
    final store = buildStore(
      details: <PokedexSpeciesDetail>[
        buildDetail(id: 'bulbasaur', learnset: null),
      ],
    );
    ...
    expect(store.learnsetById('bulbasaur')?.levelUp.single.moveId, 'vine_whip');
  ...
  testWidgets('creates an evolution locally from the dedicated tab', (
    ...
    final store = buildStore(
      details: <PokedexSpeciesDetail>[
        buildDetail(id: 'bulbasaur', evolution: null),
      ],
    );
  ...
  testWidgets('creates media references locally from the dedicated tab', (
    ...
    final store = buildStore(
      details: <PokedexSpeciesDetail>[
        buildDetail(id: 'bulbasaur', media: null),
      ],
    );
    ...
    await tester.pumpAndSettle();
    await tester.ensureVisible(
      find.byKey(const Key('pokedex-edit-media-button')),
    );
  ...
  testWidgets('shows a loading state before the species list resolves', (
    ...
    await pumpPokedexWidget(
      tester,
      container,
      child: PokedexWorkspace(loader: (_) => completer.future),
    );
    await tester.pump();
  ...
  test(
    ...
    () async {
      final tempProjectRoot = await Directory.systemTemp.createTemp(
        'pokedex_loader_test_',
      );
      try {
