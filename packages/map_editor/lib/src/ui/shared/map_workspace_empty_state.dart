...
class MapWorkspaceEmptyState extends ConsumerWidget {
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: colors.brandPrimaryBorder, width: 1.5),
                  ),
                ...
                Wrap(
                  ...
                  children: [
                    PokeMapButton(
                      ...
                      onPressed: () =>
                          showTopToolbarNewProjectDialog(context, notifier),
                      child: const Text('Créer un projet'),
                    ),
                    PokeMapButton(
                      variant: PokeMapButtonVariant.secondary,
                      onPressed: () =>
                          showTopToolbarOpenProjectDialog(context, notifier),
  ...
  void _showMapsSelectionMenu(BuildContext context, List<ProjectMapEntry> maps,
...
class _IsometricLayersPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawDottedLine(double x, double yStart, double yEnd) {
      ...
      while (y < yEnd) {
        canvas.drawLine(
            Offset(x, y), Offset(x, math.min(y + dash, yEnd)), dottedPaint);
        y += dash + gap;
      }
    }
    ...
    drawDottedLine(
        cx, cy + dyTop - h / 2, cy + dyBottom - h / 2); // Top corners
    drawDottedLine(cx + w / 2, cy + dyTop, cy + dyBottom); // Right corners
    drawDottedLine(
        cx, cy + dyTop + h / 2, cy + dyBottom + h / 2); // Bottom corners
    ...
    _drawLayer(canvas, cx, cy + dyTop, w, h,
        colors.brandPrimary.withValues(alpha: 0.16), colors.brandPrimary);
    ...
    final roofRight = Path()
      ...
      ..lineTo(houseCx + hw, houseCy + hw * h / w - hh)
      ..lineTo(houseCx + hw - hDepth / 2,
          roofPeakY + hw * h / w + hDepth / 2 * h / w)
      ..lineTo(houseCx - hDepth / 2, roofPeakY + hDepth / 2 * h / w)
      ..close();
    canvas.drawPath(
        roofRight,
  ...
  void _drawLayer(Canvas canvas, double cx, double cy, double w, double h,
