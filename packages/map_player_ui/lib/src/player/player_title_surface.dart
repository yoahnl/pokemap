...
final class PlayerTitleContinueSaveData {
  const PlayerTitleContinueSaveData({
...
@immutable
final class PlayerTitleSurfaceData {
...
class PlayerTitleSurface extends StatelessWidget {
  const PlayerTitleSurface({
    super.key,
    required this.data,
  ...
  final PlayerTitleSurfaceData data;
  ...
  void _select(PlayerTitleMenuAction action) {
    focusController?.selectFromTouch(_logicalId(action));
    onSelected(action);
  }
