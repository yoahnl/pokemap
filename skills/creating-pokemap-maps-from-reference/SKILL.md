...
Translate visual intent, not pixels. The result must remain native PokeMap data: editable layers, paths, Environment areas, placed elements, events, and project-owned assets. Treat existing collision data as a protected gameplay contract, not as visual material to regenerate.
...
- Preserve existing collision layers, masks, shapes, and gameplay zones byte-for-byte unless the owner explicitly authorizes a separate collision lot. Validate them read-only; never infer new collisions from the reference image.
...
2. Run `scripts/create_reference_brief.py`. Annotate zones, landmarks, traversable routes, occlusion, water contexts, Environment candidates, and protected cells. Record collision data as frozen unless the prompt explicitly scopes a separate collision change.
...
4. Implement deterministically in a generator or focused authoring tool. Preserve gameplay contracts, do not rewrite collisions, and mark placement origins (`authored`, `tile_index`, `environment`). If the new visual layout cannot coexist with the frozen collision contract, stop and request a separate decision instead of guessing.
...
7. Validate through the real editor session (`loadMap -> edit unrelated field -> saveActiveMap -> reload`), runtime loading, collision preservation, traversal, and two-pass rendering. Collision checks are diagnostic only and must not mutate authored data.
...
Stop when provenance, editor losslessness, frozen collisions, navigation, or visual review is unresolved. Report the blocker instead of flattening the reference, guessing collision geometry, or deleting uncertain files.
