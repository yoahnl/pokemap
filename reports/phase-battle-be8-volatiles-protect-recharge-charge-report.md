# Phase Battle BE8 — Volatiles utiles v1 + Protect / requireRecharge / chargeThenStrike / breakProtect

## 1. Résumé exécutif honnête

### Verdict

BE8 est maintenant un vrai lot moteur branché, petit mais réel.

Ce lot fait réellement ceci :
- ajoute un sous-état volatile battle local explicite pour `protect`, `mustRecharge` et un move chargé en attente ;
- ouvre dans `map_battle` un support honnête de `Protect`, `breakProtect`, `requireRecharge` et `chargeThenStrike` ;
- conserve l’ordre BE3 figé au début du tour ;
- garde le hit pipeline BE4, les types BE5, les crits BE6 et les statuts majeurs BE7 compatibles ;
- ouvre le bridge runtime uniquement sur le sous-ensemble canonique BE8 réellement exécutable ;
- garde explicitement refusés les autres volatiles et les lots BE9+.

Ce lot ne fait pas :
- pas de framework générique de volatiles ;
- pas de hooks génériques de fin de tour ;
- pas de switch pipeline ;
- pas de weather/terrain ;
- pas de semi-invulnerability, Power Herb, success decay de Protect, doubles, field state ;
- pas de support global de `applyVolatileStatus` ;
- pas de garantie que le convertisseur Showdown actuel produise déjà tout le sous-ensemble BE8 depuis les données externes.

C’est un vrai changement de code, pas un lot documentaire.

### Ce que j’ai réellement fait

- J’ai créé un petit contrat battle local `BattleVolatileState` / `BattleVolatileEvent` dans `packages/map_battle/lib/src/battle_volatile.dart`.
- J’ai propagé ce contrat dans `BattleCombatantData`, `BattleCombatant`, `BattleTurnResult`, `BattleMoveData` et `BattleMove`.
- J’ai intégré la logique moteur dans `BattleSession` : activation de Protect, blocage par Protect, bypass `breakProtect`, tour de recharge forcé, charge sur deux tours, trace des événements volatiles et nettoyage de fin de tour pour Protect.
- J’ai ajouté `PlayerBattleChoiceContinue` et `BattleActionRecharge` pour rendre honnêtes les tours forcés BE8.
- J’ai ouvert le bridge runtime exact pour :
  - `applyVolatileStatus(self, protect)`
  - `breakProtect`
  - `requireRecharge`
  - `chargeThenStrike`
- J’ai explicitement refusé les combinaisons hors contrat, notamment `chargeThenStrike + requireRecharge` sur un même move.
- J’ai ajouté des tests battle dédiés BE8, des tests bridge runtime BE8, un test seed builder et un test setup mapper d’intégration.
- J’ai ajouté un petit ajustement runtime UI dans `battle_overlay_component.dart` pour éviter le bouton `???` sur les tours forcés et pour rendre les événements volatiles visibles dans l’overlay.

### Ce que je n’ai volontairement PAS fait

- Je n’ai pas touché `packages/map_core`.
- Je n’ai pas touché `packages/map_editor`.
- Je n’ai pas ouvert un système générique de volatiles.
- Je n’ai pas ouvert `applyVolatileStatus` en général.
- Je n’ai pas ouvert Detect/Wide Guard/King's Shield ni la décroissance de succès de Protect.
- Je n’ai pas ouvert les raccourcis météo des moves chargés.
- Je n’ai pas ouvert d’interactions abilities/items.
- Je n’ai pas réécrit le convertisseur Showdown pour émettre `chargeThenStrike` automatiquement.

## 2. Pré-gates exécutés + résultats

### Commandes pré-gates réellement exécutées
...
### Résultats honnêtes des pré-gates

- `git status --short` : vert, worktree propre avant BE8.
- `git diff --stat` : vert, aucun diff avant BE8.
- `git ls-files --others --exclude-standard` : vert, aucun untracked avant BE8.
- `packages/map_battle` `dart test` : vert.
- `packages/map_battle` `dart analyze` : vert.
- `packages/map_runtime` `flutter test ...` ciblé : vert.
- `packages/map_runtime` `flutter analyze --no-pub ...` ciblé : vert.

Aucun rouge préexistant confirmé sur les pré-gates BE8.

## 3. État initial audité réel

Constats confirmés avant modification :
- `map_battle` savait déjà gérer BE3/BE4/BE5/BE6/BE7.
- Il n’existait pas encore de sous-état volatile local pour porter `Protect`, une recharge forcée ou un move chargé entre deux tours.
- `BattleTurnResult` savait tracer les exécutions et les statuts majeurs, mais pas les événements volatiles BE8.
- `RuntimeBattleMoveBridge` refusait encore explicitement `applyVolatileStatus`, `breakProtect`, `requireRecharge` et `chargeThenStrike`.
- `Protect` n’existait pas comme effet battle dédié ; dans le canonique réel, il est représenté via `PokemonMoveEffect.applyVolatileStatus(volatileStatusId: 'protect')`.
- `requireRecharge` et `breakProtect` existent bien dans le canonique.
- `chargeThenStrike` existe dans `map_core`, mais le convertisseur Showdown actuel ne le produit pas encore automatiquement pour les vrais moves à charge du catalogue importé.
- `PokemonMoveFlag.protect` n’est pas le move Protect ; c’est seulement le flag “ce move est affecté par Protect”.

## 4. Problèmes confirmés / non confirmés

### Confirmés

- Le moteur battle n’avait aucun état local pour rendre honnêtes `Protect`, la recharge forcée et la charge sur deux tours.
- Le bridge runtime rejetait encore l’intégralité du sous-ensemble BE8.
- Sans petit contrat de trace dédié, BE8 aurait réintroduit des mutations silencieuses.
- L’overlay runtime affichait `???` pour le choix forcé introduit par BE8 et ne montrait pas les événements volatiles du tour.

### Non confirmés / recadrés

- Le prompt laissait entendre qu’on pouvait potentiellement “supporter Protect / Solar Beam” de bout en bout via le pipeline Showdown existant. Ce n’est pas pleinement vrai aujourd’hui.
- `chargeThenStrike` existe bien dans le modèle canonique, mais le convertisseur Showdown réel le marque encore comme limite (`unsupported_mechanic:charge_then_strike`) au lieu de l’émettre.
- Le support runtime BE8 est donc honnête pour le sous-ensemble canonique réellement représenté dans les objets `PokemonMove`, mais pas une garantie que l’import Showdown actuel fournisse déjà tous les exemples emblématiques.

## 5. Cause racine réelle

La cause racine n’était pas un “manque de move family” mais l’absence d’une petite couche d’état intermédiaire au niveau combatant, plus une absence de traces dédiées.

Sans cette couche :
- `Protect` ne pouvait pas vivre entre deux résolutions d’actions dans le même tour ;
- `requireRecharge` ne pouvait pas forcer honnêtement le tour suivant ;
- `chargeThenStrike` ne pouvait pas porter un move en attente d’un tour à l’autre ;
- `breakProtect` n’avait rien à percer ;
- le bridge runtime n’avait aucune cible battle honnête vers laquelle projeter ces effets.

## 6. Décisions retenues / rejetées

### Retenues

- petit fichier battle local `battle_volatile.dart` plutôt qu’un gros “volatile framework” ;
- sous-état explicite sur le combattant avec trois briques seulement :
  - `protectActive`
  - `mustRecharge`
  - `pendingCharge`
- petite liste sœur `volatileEvents` dans `BattleTurnResult` au lieu de gonfler `BattleMoveExecution` ;
- `PlayerBattleChoiceContinue` pour rendre honnêtes les tours forcés ;
- `BattleActionRecharge` pour tracer un tour perdu sans bricoler un faux `Fight` ;
- bridge runtime ultra strict :
  - `applyVolatileStatus` uniquement pour `protect` sur `self`
  - `breakProtect` seulement dans la forme déterministe ciblant l’opponent
  - `requireRecharge` seulement pour un move de dégâts standard vers l’opponent
  - `chargeThenStrike` seulement pour un move de dégâts standard vers l’opponent
- refus explicite de la combinaison `chargeThenStrike + requireRecharge` sur un même move.

### Rejetées

- framework générique de volatiles : rejeté comme surdimensionné et trompeur pour BE8 ;
- event bus générique : rejeté ;
- hook system de fin de tour : rejeté ;
- ouverture large de `applyVolatileStatus` : rejetée ;
- support implicite d’un Protect-like via flags uniquement : rejeté ;
- support “on verra plus tard” des raccourcis météo / Power Herb / semi-invulnérabilité : rejeté ;
- transformation du tour forcé en faux bouton `Fight(0)` : rejetée ;
- silence UI sur les tours forcés et événements volatiles : rejeté après review séparée.

## 7. Critique explicite du prompt

### Ce qui était juste

- Le prochain lot logique après BE7 était bien une petite couche de volatiles utiles.
- `Protect`, `requireRecharge`, `chargeThenStrike` et `breakProtect` sont bien le sous-ensemble structurant cohérent.
- Il fallait absolument éviter un framework générique et rester local.
- Le besoin d’une vraie trace honnête était réel.

### Ce qui était discutable

- Le prompt suggérait implicitement que la chaîne runtime/canonique actuelle permettrait déjà de “brancher Protect / Solar Beam” de bout en bout. L’audit montre que ce n’est vrai qu’en partie.
- `chargeThenStrike` existe bien dans `map_core`, mais pas encore comme sortie normale du convertisseur Showdown actuel.
- `Protect` est bien représenté canoniquement via `applyVolatileStatus('protect')`, mais pas nécessairement disponible via tous les catalogues importés existants si leur `engineSupportLevel` reste plus faible en amont.

### Ce qui aurait été dangereux si suivi aveuglément

- Ouvrir tout `applyVolatileStatus` sous prétexte de supporter Protect.
- Faire semblant que `PokemonMoveFlag.protect` voulait dire “c’est un move Protect”.
- Élargir BE8 à un pseudo-système complet des moves à charge, avec météo, herb, semi-invulnérabilité, etc.

### Recadrage retenu

- BE8 ouvre honnêtement le moteur battle et le bridge runtime pour le sous-ensemble canonique exact.
- BE8 ne promet pas que le pipeline Showdown réel produise déjà automatiquement tous les exemples emblématiques.
- J’ai ajouté un mini-ajustement d’overlay runtime, non parce que BE8 deviendrait un lot UI, mais parce que `PlayerBattleChoiceContinue` sans texte honnête et sans affichage des `volatileEvents` aurait remis du mensonge côté runtime.

Pourquoi ce recadrage est meilleur :
- il garde le lot petit ;
- il n’invente pas du faux support catalogue ;
- il évite de laisser une surface runtime trompeuse alors que le moteur devient plus précis.

## 8. Périmètre inclus / exclu

### Inclus

- `packages/map_battle` : nouveau sous-état volatile local, choix forcé, action de recharge, résolution moteur BE8, trace volatile.
- `packages/map_runtime` : bridge BE8 exact, petite amélioration d’overlay pour honnêteté de surface, tests runtime ciblés.
...
- `packages/map_editor`
- switch pipeline
- field/side/slot state
- weather / terrain
- doubles
- hooks génériques
- volatiles hors sous-ensemble BE8
- `slp`, `frz`, confusion
- Power Herb / success decay / semi-invulnerability
- abilities / items
- framework de queue générique
- refonte générale du moteur
...
- `packages/map_battle/lib/src/battle_volatile.dart`
  - nouveau contrat local BE8 minimal pour l’état volatile et sa trace.
- `packages/map_battle/lib/src/battle_action.dart`
  - nouveau choix `Continue` et nouvelle action `Recharge` pour rendre honnêtes les tours forcés.
- `packages/map_battle/lib/src/battle_move.dart`
  - nouveaux champs move BE8 (`selfVolatileStatus`, `breaksProtect`, `requiresRecharge`, `chargeThenStrikeEffect`).
- `packages/map_battle/lib/src/battle_setup.dart`
  - propagation setup du sous-état volatile et des nouveaux champs move.
- `packages/map_battle/lib/src/battle_state.dart`
  - état combatant enrichi pour porter les volatiles BE8.
- `packages/map_battle/lib/src/battle_resolution.dart`
  - ajout de `volatileEvents` au résultat de tour.
- `packages/map_battle/lib/src/battle_session.dart`
  - cœur moteur BE8.
- `packages/map_battle/lib/map_battle.dart`
  - export public du nouveau contrat local.
- `packages/map_battle/test/battle_session_test.dart`
  - vérifications de propagation et de surface de choix forcé.
- `packages/map_battle/test/battle_volatiles_test.dart`
  - preuves dédiées BE8.
- `packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart`
  - ouverture bridge exacte BE8, refus explicites conservés, refus de combinaison hors contrat.
- `packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart`
  - correction d’honnêteté de surface pour les tours forcés et les événements volatiles.
- `packages/map_runtime/test/runtime_battle_move_bridge_test.dart`
  - preuves bridge BE8 positives et négatives.
- `packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart`
  - preuve que le seed builder ne reperd pas les champs BE8.
- `packages/map_runtime/test/runtime_battle_setup_mapper_test.dart`
  - preuve que le mapper conserve BE8 jusqu’au battle.
...
sed -n '261,620p' packages/map_battle/lib/src/battle_session.dart
sed -n '1,320p' packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart
sed -n '1,240p' packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart
sed -n '1,260p' packages/map_battle/lib/src/battle_volatile.dart
sed -n '1,260p' packages/map_battle/lib/src/battle_move.dart
sed -n '1,260p' packages/map_battle/lib/src/battle_setup.dart
sed -n '1,220p' packages/map_battle/lib/src/battle_action.dart
...
sed -n '620,980p' packages/map_battle/lib/src/battle_session.dart
sed -n '780,1185p' packages/map_battle/lib/src/battle_session.dart
sed -n '1186,1320p' packages/map_battle/lib/src/battle_session.dart
sed -n '320,760p' packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart
sed -n '1,260p' packages/map_core/lib/src/models/pokemon_move_effect.dart
sed -n '360,620p' packages/map_editor/lib/src/application/services/showdown_move_catalog_converter.dart
sed -n '1,260p' packages/map_runtime/test/runtime_battle_move_bridge_test.dart
sed -n '261,760p' packages/map_runtime/test/runtime_battle_move_bridge_test.dart
...
sed -n '1,260p' packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart
rg -n "protect|Protect|requireRecharge|chargeThenStrike|breakProtect|applyVolatileStatus|volatile|hyper_beam|solar_beam|feint|detect" packages/map_core packages/map_runtime packages/map_editor packages/map_battle
rg -n "enum PokemonMoveFlag|protect|recharge|charge|breakProtect|volatileStatus|engineSupportLevel|usesStandardDamageFlow|flags|effects" packages/map_core/lib/src/models/pokemon_move.dart packages/map_editor/lib/src/application/services/showdown_move_catalog_converter.dart
sed -n '250,320p' packages/map_editor/lib/src/application/services/showdown_move_catalog_converter.dart
sed -n '1180,1215p' packages/map_editor/lib/src/application/services/showdown_move_catalog_converter.dart
rg -n "_writePokemonFixtures|_moveEntry|_defaultEffectsForMove|moves.json|protect|hyper_beam|solar_beam|feint" packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart packages/map_runtime/test/runtime_battle_setup_mapper_test.dart
sed -n '520,980p' packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart
sed -n '520,1040p' packages/map_runtime/test/runtime_battle_setup_mapper_test.dart
rg -n "_getChoiceText|BattleActionRecharge|PlayerBattleChoiceContinue|currentTurn|executions|statusEvents|volatileEvents" packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart
sed -n '240,520p' packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart
wc -l <touched files>
```

### Validation intermédiaire / finale

```bash
cd packages/map_battle && /opt/homebrew/bin/dart analyze
cd packages/map_battle && /opt/homebrew/bin/dart test
cd packages/map_runtime && /opt/homebrew/bin/flutter analyze --no-pub lib/src/application/runtime_battle_move_bridge.dart lib/src/application/runtime_battle_combatant_seed_builder.dart lib/src/application/runtime_battle_setup_mapper.dart lib/src/application/runtime_pokemon_species_loader.dart lib/src/presentation/flame/battle_overlay_component.dart test/runtime_battle_move_bridge_test.dart test/runtime_battle_combatant_seed_builder_test.dart test/runtime_battle_setup_mapper_test.dart test/runtime_move_catalog_loader_test.dart test/runtime_pokemon_species_loader_test.dart test/runtime_pokemon_learnset_loader_test.dart test/runtime_battle_outcome_apply_test.dart test/wild_battle_end_to_end_flow_test.dart test/playable_map_game_whiteout_lite_test.dart
...
cd packages/map_runtime && /opt/homebrew/bin/flutter test test/runtime_battle_setup_mapper_test.dart --plain-name 'RuntimeBattleSetupMapper maps a supported requireRecharge move and keeps the forced follow-up honest in battle'
```

### Format

```bash
cd packages/map_battle && /opt/homebrew/bin/dart format lib/map_battle.dart lib/src/battle_action.dart lib/src/battle_move.dart lib/src/battle_resolution.dart lib/src/battle_session.dart lib/src/battle_setup.dart lib/src/battle_state.dart lib/src/battle_volatile.dart test/battle_session_test.dart test/battle_volatiles_test.dart
cd packages/map_runtime && /opt/homebrew/bin/dart format lib/src/application/runtime_battle_move_bridge.dart lib/src/presentation/flame/battle_overlay_component.dart test/runtime_battle_combatant_seed_builder_test.dart test/runtime_battle_move_bridge_test.dart test/runtime_battle_setup_mapper_test.dart
```

### Outils agent / review

- sub-agent d’audit/design : Bohr (`019d9690-fc95-7e01-a274-d435630f6ab7`)
- reviewer séparé : Rawls (`019d966e-e029-7890-b485-e3241d81f7c7`)

## 12. Résultats réels de format / analyze / tests

### Format

- `packages/map_battle` format ciblé : vert.
- `packages/map_runtime` format ciblé : vert.

### Analyze

- `packages/map_battle` `dart analyze` final : vert.
- `packages/map_runtime` `flutter analyze --no-pub ...` final : vert.

### Tests

- `packages/map_battle` `dart test` final : vert.
- `packages/map_runtime` `flutter test ...` ciblé final : vert.
- test ciblé `runtime_battle_setup_mapper_test.dart --plain-name ...requireRecharge...` : vert après correction du scénario.

## 13. Incidents rencontrés

- `battle_session.dart` était dans un état intermédiaire déjà amorcé au moment de la reprise ; il a fallu terminer proprement les classes privées de résultat et la propagation `volatileEvents`.
- Premier passage runtime :
  - un test de rejet `applyVolatileStatus` attendait encore l’ancien message générique BE7 ;
  - le bridge rejetait désormais plus précisément par scope, ce qui est correct ; j’ai mis le test à jour.
- Premier passage du test d’intégration `requireRecharge` :
  - le scénario ne prouvait pas honnêtement le tour forcé suivant ;
  - d’abord parce que la cible pouvait mourir trop tôt ;
  - ensuite parce que le RNG scripté était trop court pour les checks critiques ;
  - j’ai corrigé le setup pour garder un scénario vivant et déterministe.
- Review séparée :
  - Rawls a trouvé un vrai scope leak sur la combinaison `chargeThenStrike + requireRecharge` et une faiblesse d’honnêteté dans l’overlay ;
  - les deux points ont été intégrés.

Aucun incident Git. Aucun outil n’a nécessité d’escalade manuelle.
...
```text
 ...
 packages/map_battle/lib/src/battle_move.dart       |  51 +++
 packages/map_battle/lib/src/battle_resolution.dart |  13 +
 packages/map_battle/lib/src/battle_session.dart    | 454 +++++++++++++++++++--
 packages/map_battle/lib/src/battle_setup.dart      |  40 ++
 packages/map_battle/lib/src/battle_state.dart      |  40 ++
 packages/map_battle/test/battle_session_test.dart  | 137 ++++++-
 .../application/runtime_battle_move_bridge.dart    | 213 +++++++++-
 .../flame/battle_overlay_component.dart            |  44 +-
 ...runtime_battle_combatant_seed_builder_test.dart |  70 ++++
 .../test/runtime_battle_move_bridge_test.dart      | 169 +++++++-
 .../test/runtime_battle_setup_mapper_test.dart     |  90 +++-
 13 files changed, 1274 insertions(+), 76 deletions(-)
```
...
- [x] Protect est réellement supporté dans le sous-ensemble BE8
- [x] `requireRecharge` existe réellement
- [x] `chargeThenStrike` existe réellement
- [x] `breakProtect` existe réellement
- [x] je n’ai pas rouvert BE9+ en douce
- [x] je n’ai pas ouvert de framework générique
- [x] les refus explicites hors scope restent en place
...
- [x] le report contient le contenu complet des fichiers touchés (hors le report lui-même)

## 16. Retour du sub-agent d’audit/design

Bohr a confirmé et/ou recadré les points suivants :
- `breakProtect` et `requireRecharge` existent bien canoniquement.
- `chargeThenStrike` existe bien dans `map_core`, mais le convertisseur Showdown actuel ne l’émet pas encore réellement ; `solar_beam` reste donc bloqué en amont dans le pipeline réel importé.
- `Protect` n’est pas un effet dédié ; dans ce repo, il est représenté via `PokemonMoveEffect.applyVolatileStatus(volatileStatusId: 'protect')`, généralement sur `self`.
- `PokemonMoveFlag.protect` ne veut pas dire “c’est le move Protect”.
- Le plus petit contrat battle honnête est un sous-état combatant, pas un framework de volatiles.

Ce retour a été retenu.

## 17. Retour du reviewer séparé

Reviewer séparé : Rawls.

Findings remontés :
1. il manquait un refus explicite pour un move non canonique combinant `chargeThenStrike` et `requireRecharge` ;
2. l’overlay runtime restait trop discret sur les événements volatiles et sur le vrai sens du choix forcé `Continue`.

Ces remarques étaient valides.

## 18. Corrections appliquées après review

- ajout d’un refus explicite `unsupported_combined_charge_then_recharge` dans `RuntimeBattleMoveBridge` ;
- ajout d’un test runtime dédié pour verrouiller ce refus ;
- amélioration minimale de `battle_overlay_component.dart` :
  - affichage spécifique de `Continue (recharge)` / `Continue (libérer la charge)` ;
  - affichage des `volatileEvents` du tour dans le texte de résultat.

## 19. Autocritique finale

Le lot est solide, mais il reste des limites assumées :
- le moteur BE8 est honnête localement, mais le vrai pipeline Showdown/import ne fournit pas encore tout le sous-ensemble canoniquement ;
- l’overlay runtime est maintenant moins trompeur, mais il ne raconte toujours pas toute la richesse des `statusEvents` BE7 dans la même finesse que les volatiles BE8 ;
- la sémantique de `requireRecharge` reste volontairement simplifiée : elle n’essaie pas de reproduire tous les cas historiques Pokémon ;
- `chargeThenStrike` reste volontairement simple : pas de météo, pas de semi-invulnérabilité, pas de Power Herb.

Je considère néanmoins le lot terminé et défendable : il ouvre une vraie couche moteur, sans faux support et sans framework prématuré.
...
void main() {
  group('BattleSession', () {
    ...
    test(
        'createBattleSession preserves the additional honest battle contract fields transported by BE1 through BE8',
        () {
      final setup = BattleSetup(
        playerPokemon: BattleCombatantData(
          speciesId: 'pikachu',
