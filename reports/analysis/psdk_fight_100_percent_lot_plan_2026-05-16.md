...
**Status 2026-05-16:** in progress. `s_sleep_talk` now has a dedicated copy/call behavior for the local supported slice: it fails before PP while awake, allows Comatose, filters the PSDK excluded move list, consumes the generic RNG stream, and executes the selected user move without spending that selected move's PP. `s_metronome` now selects from a local supported move pool with the PSDK exclusion list, consumes the generic RNG stream, and executes the selected move without spending that selected move's PP. `s_assist` now filters the PSDK exclusion list, fails after Assist PP when no teammate move can be selected, consumes the generic RNG stream, and executes the selected same-bank teammate move without spending copied-move PP for the local supported slice. `s_mimic` now fails before PP when the target has no supported last successful move or the last move is PSDK-excluded, and replaces the selected user move slot with the target move at 5/5 PP for the local supported slice. `s_sketch` now fails before PP when the target has no local last move, the user is transformed, or the selected slot no longer contains Sketch, then replaces the selected slot with the target move at full PP for the in-battle slice. `s_mirror_move` now covers Mirror Move and Copycat for the local supported slice: Mirror Move reads the target's last attempted move from the current or previous turn, Copycat reads another alive battler's last attempted move, both fail after caller PP when nothing can be copied, and both execute the copied move without spending copied-move PP. `s_instruct` now fails after caller PP when the target has no instructable last attempted move, applies the PSDK excluded move list plus local forced/charge blockers, spends the repeated target move PP, marks the target with `instruct`, and executes the repeated move inline against its first historical target for the local supported slice. `s_me_first` now reads the target's still-pending announced action, fails after caller PP when the target already moved or announced an excluded move, calls the announced move without spending copied-move PP, and applies a local 1.5 power boost plus 100 accuracy for the supported slice. This remains partial because local move history still lacks PSDK `original_move` metadata, dynamic action insertion, `attack_order`, Studio `is_mirror_move` import, exact PSDK party-backed `trainer_battlers` / retargeting semantics, and Me First uses a local power boost approximation instead of a dedicated PSDK `calc_mod2` multiplier.
...
damage earlier in the current turn. Opposing major-status moves are blocked by
Substitute for the non-authentic local slice, while non-move `effect:` /
`status:` status sources remain outside this interception. Regression coverage
lives in `substitute_focus_punch_test.dart`. The parity gate remains at
262 / 728 strict attacks and 63 / 330 ported battle methods because volatile
status/stat blocking, sound/Infiltrator/authentic exceptions, Focus Punch
pre-attack preparation effect, and full Substitute move-interaction parity are
still open.
