...
BE7 est maintenant un vrai lot moteur, pas un support cosmétique.

Ce que le lot fait réellement :
- `map_battle` porte désormais un vrai état local de statut majeur pour `par`, `brn`, `psn`, `tox`.
- `RuntimeBattleMoveBridge` n'éjecte plus `applyStatus` en bloc ; il accepte uniquement le sous-ensemble honnêtement exécutable par le moteur BE7.
- le moteur applique désormais :
  - la paralysie avec malus de vitesse réel et chance de full paralysis ;
  - la brûlure avec résiduel et malus sur les dégâts physiques ;
  - le poison avec résiduel ;
  - le poison toxique avec compteur local et résiduel croissant ;
  - une phase explicite de résiduel de fin de tour ;
  - une trace minimale mais honnête des applications de statut, blocages utiles et résiduels.
- les statuts hors scope (`slp`, `frz`) et `applyVolatileStatus` restent refusés explicitement.

Ce que le lot ne fait volontairement pas :
- pas de volatiles génériques ;
- pas de `slp`, `frz`, confusion ;
- pas de switch pipeline ni reset toxique sur switch ;
- pas d'immunités complètes de statut par type/ability/item ;
- pas de weather, terrain, side state, hooks génériques ;
- pas d'event bus ;
- pas de refonte globale de `BattleSession`.

Verdict honnête :
- oui, `applyStatus` est maintenant vraiment branché sur le moteur ;
- oui, `par/brn/psn/tox` existent réellement comme couche battle ;
- non, ce n'est pas encore un système de statuts Pokémon complet ;
- le scope est resté petit et local, mais le diff est mécaniquement plus large que BE6-mini-fix-2 parce qu'il fallait ouvrir une vraie phase de résiduel et une vraie trace de statut.

## Ce que j'ai réellement fait
- ajouté un contrat battle local `battle_status.dart` pour les statuts majeurs, l'effet move `applyStatus` et la trace minimale de statut ;
- étendu `BattleCombatantData`, `BattleCombatant`, `BattleMoveData` et `BattleMove` pour porter l'état/effet nécessaires ;
- ajouté la logique moteur de paralysie, brûlure, poison, toxique, et résiduels de fin de tour ;
- conservé `currentTurn` même sur les tours terminaux résolus, afin que les K.O. au résiduel restent visibles dans la trace ;
- ouvert `applyStatus` dans le bridge runtime pour le seul sous-ensemble `targetScope: target`, `move target -> opponent`.

## Ce que je n'ai volontairement pas fait
- je n'ai pas modifié `packages/map_core` ;
- je n'ai pas modifié `packages/map_editor` ;
- je n'ai pas touché `battle_rng.dart`, parce que le seam BE4/BE6 suffisait déjà pour la paralysie et les effets probabilistes de statut ;
- je n'ai pas ouvert d'immunité de statut pour les moves de statut purs comme Thunder Wave sur un type sol ;
- je n'ai pas introduit de trace pour les non-procs probabilistes de statut ;
- je n'ai pas rouvert `applyVolatileStatus`, `slp`, `frz`, weather, terrain, switch, multi-hit ou hooks génériques.

# 2. Pré-gates exécutés + résultats

## Pré-gates exécutés avant code
...
- `packages/map_runtime :: flutter test ...` : vert.
- `packages/map_runtime :: flutter analyze --no-pub ...` : vert.

## Classification honnête
- pré-gates battle : verts.
- pré-gates runtime : verts.
- incident d'outillage non bloquant : `flutter analyze` a attendu la libération du startup lock parce que le test runtime et l'analyse runtime avaient été lancés en parallèle. Le résultat final est resté vert.

# 3. État initial audité réel

Constats confirmés avant modification :
- `BattleCombatant` ne portait aucun statut majeur.
- `BattleTurnResult` ne portait aucune trace de statut ni de résiduel.
- `BattleMoveData` / `BattleMove` ne transportaient aucun effet `applyStatus`.
- `BattleSession` ne connaissait ni blocage d'action par statut, ni résiduels de fin de tour.
- `RuntimeBattleMoveBridge` rejetait encore `applyStatus` en bloc via `bridgeLimit=unsupported_effect_kind:apply_status`.
- les fixtures runtime `thunder_wave` existaient déjà, mais uniquement comme preuve de rejet du bridge.
- BE3/BE4/BE5/BE6 avaient déjà fourni les briques nécessaires : ordre figé au début du tour, RNG local, hit pipeline, PP, type damage, crits.

Constat supplémentaire découvert pendant l'audit :
- même si le prompt parlait surtout de statuts/résiduels, l'observabilité restait encore incomplète sur les tours terminaux, car `applyChoice()` jetait `currentTurn` dès qu'un outcome existait. Pour des K.O. au résiduel, cela aurait re-silencié la trace. J'ai recadré ce point localement dans `BattleSession`.

# 4. Problèmes confirmés / non confirmés

## Confirmés
- absence réelle d'état de statut majeur côté battle ;
- rejet intégral de `applyStatus` côté bridge runtime -> battle ;
- absence réelle de phase de résiduel de fin de tour ;
- absence réelle de trace minimale pour les statuts/résiduels ;
- perte de la trace sur les tours terminaux résolus si `currentTurn` restait null en fin de combat.

## Non confirmés / recadrés
- le prompt laissait ouverte l'idée d'immunités de statut plus larges ; je ne l'ai pas ouverte. Le repo n'a pas encore de couche solide pour type immunities de status moves purs, items, abilities, cure/cleanse, etc.
- le prompt suggérait une lecture potentiellement plus large de `applyStatus`; j'ai resserré à `targetScope: target` et `move target` cohérent avec `opponent`. C'est plus petit et plus honnête.
- je n'ai pas eu besoin de toucher `packages/map_runtime/lib/src/application/runtime_battle_combatant_seed_builder.dart` ni `runtime_battle_setup_mapper.dart` en production ; les extensions de contrat battle étaient optionnelles et compatibles. Seuls leurs tests ont été mis à jour.

# 5. Cause racine réelle

La vraie cause n'était pas seulement “le bridge refuse `applyStatus`”.

La cause racine était une absence de couche battle locale capable de représenter et de consommer un statut majeur de bout en bout :
- pas d'état sur le combattant ;
- pas de contrat `applyStatus` battle minimal ;
- pas de blocage d'action par statut ;
- pas de résiduels de fin de tour ;
- pas de trace minimale dédiée.

Tant que cette couche n'existait pas, ouvrir `applyStatus` dans le bridge aurait été mensonger.

# 6. Décisions retenues / rejetées

## Retenues
- créer un petit fichier battle local `battle_status.dart` plutôt que d'éparpiller statuts, effets et événements dans `battle_session.dart` ;
- porter un seul slot `majorStatus` nullable sur `BattleCombatant` ;
- transporter un seul `majorStatusEffect` optionnel sur `BattleMoveData` / `BattleMove` ;
- ajouter `statusEvents` à `BattleTurnResult` plutôt que grossir `BattleMoveExecution` pour couvrir aussi les résiduels ;
- supporter seulement `par`, `brn`, `psn`, `tox` ;
- supporter seulement `applyStatus` avec `targetScope: target` et cible battle `opponent` ;
- utiliser les politiques explicites suivantes :
  - `par`: vitesse × `0.5`, full paralysis `25%` ;
  - `brn`: dégâts physiques × `0.5`, résiduel `1/16 max HP` ;
  - `psn`: résiduel `1/8 max HP` ;
  - `tox`: résiduel `n/16 max HP`, compteur initial `1`, incrément si le combattant survit ;
- conserver la dernière garde observable sur les tours terminaux en gardant `currentTurn` après un tour qui finit le combat.

## Rejetées
- système générique de hooks/résiduels de fin de tour ;
- volatiles génériques ;
- immunités de statut complètes par type/ability/item ;
- ouverture de `slp`, `frz`, confusion ;
- refonte de `BattleMoveExecution` en “journal” complet ;
- modification de `battle_rng.dart` : la couche existante suffisait ;
- support de `applyStatus` sur `self` ou autres scopes.

# 7. Critique explicite du prompt

## Ce qui était juste
- le vrai prochain lot cohérent après BE6 était bien une première couche de statuts majeurs honnête ;
- les seams proposés (`battle_state`, `battle_session`, `battle_resolution`, `battle_move`, `battle_setup`, `runtime_battle_move_bridge`) étaient globalement les bons ;
- l'insistance sur une petite couche battle locale, sans framework de hooks ni volatiles génériques, était saine.

## Ce qui était discutable
- le prompt n'appelait pas explicitement le trou d'observabilité sur les tours terminaux (`currentTurn` jeté quand un outcome existe). Sans corriger ça, le lot aurait quand même menti sur les résiduels terminaux.
- le prompt laissait flotter la question des immunités de statut. Ouvrir Thunder Wave “comme dans Pokémon” sur les immunités de type aurait demandé une couche plus large que ce repo n'a pas encore.

## Ce qui aurait été dangereux si suivi aveuglément
- introduire un système d'immunités de statut plus large “tant qu'à faire” ;
- créer un mécanisme général de fin de tour ou un event bus ;
- forcer `applyStatus` sur plusieurs scopes sans vraie sémantique de ciblage battle.

## Ce que j'ai recadré
- j'ai limité `applyStatus` à `targetScope: target` et `BattleMoveTarget.opponent` ;
- j'ai explicitement refusé d'ouvrir les immunités de statut pour les moves de statut purs ;
- j'ai ajouté la conservation de `currentTurn` sur les tours terminaux résolus pour que la trace BE7 reste visible.

## Pourquoi ce recadrage est meilleur pour ce repo réel
- il reste cohérent avec le moteur 1v1 minimal déjà livré ;
- il évite d'ouvrir une couche de règles que le repo ne peut pas encore soutenir honnêtement ;
- il ferme un vrai trou d'observabilité au lieu de laisser une feature “verte” mais silencieuse quand le combat se termine sur un résiduel.
...
- `applyVolatileStatus`
- `slp`, `frz`, confusion
- weather / terrain / side state / slot state
- switch pipeline
- items / abilities
- status immunities larges
- framework de hooks de fin de tour
...
- `/Users/karim/Project/pokemonProject/reports/phase-battle-be7-major-statuses-report.md`
...
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/map_battle.dart`
  - export du nouveau contrat battle local de statut.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_status.dart`
  - nouveau petit contrat local pour l'état majeur, l'effet `applyStatus` battle et la trace minimale.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_move.dart`
  - transport de `majorStatusEffect` sur le move battle actif.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_setup.dart`
  - transport de `majorStatusEffect` dans le setup et ajout d'un `majorStatus` optionnel pour les call sites battle directs/tests.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_state.dart`
  - vrai state de statut majeur sur le combattant et helper immutable associé.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_resolution.dart`
  - ajout de `statusEvents` au résultat de tour.
- `/Users/karim/Project/pokemonProject/packages/map_battle/lib/src/battle_session.dart`
  - logique moteur BE7 : paralysie, application de statut, résiduels, burn damage drop, trace, conservation de `currentTurn` sur tours terminaux.
- `/Users/karim/Project/pokemonProject/packages/map_battle/test/battle_session_test.dart`
  - preuve ciblée que le setup conserve bien le nouveau contrat status.
- `/Users/karim/Project/pokemonProject/packages/map_battle/test/battle_move_effects_test.dart`
  - preuves métier BE7 sur `par/brn/psn/tox`, résiduels, traces, non-régression.
- `/Users/karim/Project/pokemonProject/packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart`
  - ouverture stricte et limitée de `applyStatus` au sous-ensemble BE7.
- `/Users/karim/Project/pokemonProject/packages/map_runtime/test/runtime_battle_move_bridge_test.dart`
  - preuves de projection runtime du sous-ensemble autorisé et des refus maintenus.
- `/Users/karim/Project/pokemonProject/packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart`
  - preuve que `thunder_wave` n'est plus rejeté et traverse honnêtement le seed builder.
- `/Users/karim/Project/pokemonProject/packages/map_runtime/test/runtime_battle_setup_mapper_test.dart`
  - preuve que le mapper runtime -> battle ne perd pas l'effet de statut et qu'une vraie session battle peut le consommer.
...
cd packages/map_battle && /opt/homebrew/bin/dart format lib/map_battle.dart lib/src/battle_move.dart lib/src/battle_setup.dart lib/src/battle_state.dart lib/src/battle_status.dart lib/src/battle_resolution.dart lib/src/battle_session.dart test/battle_session_test.dart test/battle_move_effects_test.dart
...
## Commandes d'audit ciblé réellement exécutées
```bash
sed -n '1,240p' packages/map_battle/lib/src/battle_state.dart
...
sed -n '1,260p' packages/map_battle/lib/src/battle_rng.dart
sed -n '1,220p' packages/map_core/lib/src/models/pokemon_move_effect.dart
rg -n "thunder_wave|applyStatus|unsupported_effect_kind:apply_status|slp|frz|tox|psn|brn|par" packages/map_runtime/test packages/map_battle/test
sed -n '1,260p' packages/map_battle/lib/src/battle_session.dart
sed -n '260,620p' packages/map_battle/lib/src/battle_session.dart
sed -n '620,1040p' packages/map_battle/lib/src/battle_session.dart
...
sed -n '340,560p' packages/map_runtime/test/runtime_battle_move_bridge_test.dart
sed -n '260,360p' packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart
sed -n '660,730p' packages/map_runtime/test/runtime_battle_setup_mapper_test.dart
sed -n '560,690p' packages/map_runtime/test/runtime_battle_combatant_seed_builder_test.dart
sed -n '1040,1165p' packages/map_runtime/test/runtime_battle_setup_mapper_test.dart
sed -n '1,240p' packages/map_battle/lib/map_battle.dart
sed -n '1,260p' packages/map_runtime/lib/src/application/runtime_battle_combatant_seed_builder.dart
sed -n '1,240p' packages/map_runtime/lib/src/application/runtime_battle_setup_mapper.dart
```

# 12. Résultats réels de format / analyze / tests

## Format
- `packages/map_battle :: dart format ...` : vert.
- `packages/map_runtime :: dart format ...` : un premier run a échoué sur une erreur de syntaxe introduite dans `runtime_battle_setup_mapper_test.dart`; après correction locale, le format est redevenu vert.
...
- pré-gates runtime : `flutter analyze` a attendu un startup lock parce qu'un `flutter test` parallèle était encore en cours. Résultat final vert, incident purement outillage.
- premier format runtime post-implémentation : erreur de syntaxe dans `runtime_battle_setup_mapper_test.dart` due à une parenthèse en trop dans mon patch. Corrigé immédiatement avant `analyze`.
- génération du report : un essai raté de génération shell a créé un fichier vide non désiré `opponent` à la racine du repo. Je l'ai supprimé avant clôture ; ce n'était pas un artefact du lot BE7 lui-même.
- aucun rouge structurel préexistant n'a été découvert dans les pré-gates ciblés.
- sub-agent audit/design existant utile ; reviewer séparé utile et sans finding bloquant.
...
?? packages/map_battle/lib/src/battle_status.dart
?? reports/phase-battle-be7-major-statuses-report.md
```
...
```text
 ...
 packages/map_battle/lib/src/battle_session.dart    | 401 ++++++++++++++-
 packages/map_battle/lib/src/battle_setup.dart      |  29 ++
 packages/map_battle/lib/src/battle_state.dart      |  37 ++
 .../map_battle/test/battle_move_effects_test.dart  | 556 +++++++++++++++++++++
 packages/map_battle/test/battle_session_test.dart  |  45 ++
 .../application/runtime_battle_move_bridge.dart    |  78 ++-
 ...runtime_battle_combatant_seed_builder_test.dart |  47 +-
 .../test/runtime_battle_move_bridge_test.dart      |  94 ++--
 .../test/runtime_battle_setup_mapper_test.dart     |  71 +--
 12 files changed, 1269 insertions(+), 118 deletions(-)
```
...
- [x] j'ai introduit un vrai state de statut majeur battle
- [x] `par`, `brn`, `psn`, `tox` sont réellement portés et consommés
- [x] `applyStatus` est supporté honnêtement pour le sous-ensemble autorisé
- [x] `slp`, `frz`, `applyVolatileStatus` et le reste hors scope restent refusés explicitement
- [x] les résiduels de fin de tour existent réellement
- [x] la trace du tour ne ment pas sur les statuts et résiduels
- [x] je n'ai pas ouvert weather/terrain/switch/volatiles en douce
...
Sub-agent utilisé : `Beauvoir` (`019d965a-400b-7da1-bae5-3708cc49ce07`).

Retour utile retenu :
- le plus petit design honnête était bien un seul slot de statut majeur sur le combattant ;
- il fallait éviter un framework de hooks et préférer une petite phase explicite de résiduel de fin de tour ;
- les seams naturels étaient `battle_state.dart`, `battle_session.dart`, `battle_resolution.dart`, `runtime_battle_move_bridge.dart` ;
- il fallait faire attention à `par` : l'ordre du tour reste figé au début du tour, mais une paralysie déjà présente peut toujours bloquer l'action quand elle arrive à son tour de résolution.

Décision : retour retenu presque tel quel.

# 17. Retour du reviewer séparé

Reviewer utilisé : `Rawls` (`019d966e-e029-7890-b485-e3241d81f7c7`).

Retour reçu :
> No blocking findings.
>
> I checked the BE7 status pipeline end to end in the touched battle/runtime files, including `par/brn/psn/tox`, `applyStatus` bridging, turn order, residuals, and the added tests. The implementation stays within the requested subset, the status order is coherent with the existing BE3/BE4/BE5/BE6 pipeline, and I didn’t find an obvious battle/runtime regression or a test that clearly overclaims beyond what the code actually does.
>
> Residual risk, but not a blocking finding: the implementation still does not try to model broader status immunities or failed-proc traces beyond the supported subset, which is consistent with the scope you asked for.

# 18. Corrections appliquées après review

Aucune correction de code supplémentaire n'a été nécessaire après la review séparée, car le reviewer n'a remonté aucun finding bloquant.

J'ai néanmoins gardé explicitement dans le report le risque résiduel signalé par le reviewer :
- pas d'immunités de statut plus larges ;
- pas de trace des non-procs probabilistes.

# 19. Autocritique finale

Ce qui est solide :
- le lot ouvre une vraie couche moteur, pas un pont cosmétique ;
- le bridge runtime ne ment plus sur `applyStatus` pour `par/brn/psn/tox` ;
- les effets battle demandés existent réellement et sont testés ;
- la trace de tour couvre désormais les applications, blocages et résiduels utiles ;
- le lot n'a pas besoin d'une nouvelle stack ni d'un nouveau framework.

Ce qui reste fragile ou délibérément incomplet :
- il n'y a toujours pas d'immunité de statut pour les moves de statut purs ;
- les non-procs probabilistes de statut ne génèrent pas de trace spécifique ;
- les K.O. doubles au résiduel gardent la politique d'outcome historique du moteur (l'ennemi K.O. est vérifié avant le joueur), faute de système de draw plus riche ;
- il n'y a toujours pas de cure, de switch, de reset toxique sur switch, ni de write-back hors combat des statuts ;
- `BattleCombatantData.majorStatus` existe surtout pour les call sites battle directs et les tests ; le runtime principal n'en a pas encore besoin.

Si je devais critiquer le lot lui-même :
- le diff sur `battle_session.dart` est substantiel ; il reste local, mais c'est le vrai centre de gravité du lot ;
- garder `currentTurn` sur les tours terminaux résolus était indispensable, mais c'est un petit recadrage supplémentaire que le prompt n'annonçait pas explicitement.
