# Phase 11A — Mini-Fix 2 Report

## 1. Resume executif honnete

Ce mini-fix reste strictement limite a la phase 11A existante.

Constat audite :
- le bug `skipExisting` + `media.json` deja present etait reel ;
- le pipeline continuait a telecharger des assets binaires meme quand l'artefact `media.json` etait en action `skip` ;
- cela pouvait produire des assets orphelins, parce que le `media.json` conserve n'etait pas reecrit dans ce run ;
- la logique `content-type` incompatible non-GIF etait deja correcte cote comportement, mais elle n'etait pas blindee par des tests suffisants.

Correction retenue :
- si l'artefact `media.json` est `skip`, le pipeline n'essaie plus de telecharger de nouveaux assets media binaires pour cette espece dans ce run ;
- un warning explicite est emis ;
- les branches `content-type` incompatible pour image et cry sont maintenant verrouillees par tests.

## 2. Bugs corriges exactement

1. `skipExisting` + `media.json` deja present :
- avant fix, `media.json` pouvait etre conserve (`skip`) pendant que des portraits/sprites/cries etaient quand meme telecharges ;
- apres fix, aucun nouvel asset binaire media n'est ecrit si `media.json` est `skip`.

2. `content-type` incompatible non-GIF :
- audit : le code rejetait deja les types incompatibles ;
- mini-fix : ajout de tests cibles pour prouver et verrouiller :
  - image incompatible sans asset local ;
  - image incompatible avec asset local en `overwriteExisting` ;
  - cry incompatible sans asset local.
...
- `reports/phase-11a-mini-fix-2-report.md`
...
- aucun nouveau use case externe parallele
- aucune UI / provider modifie(e)
- aucun changement de `project.json`
- aucun parsing `abilities.js`

## 5. Sub-agents utilises et decisions retenues

### Boyle — audit contradictoire `skipExisting`
Conclusion :
- le bug etait reel ;
- la branche `_downloadBestEffortAssets(...)` restait executee meme quand `mediaPlan.action == skip` ;
- plus petit fix defendable : court-circuiter la phase de download binaire quand `media.json` est `skip`.

Decision retenue :
- retenu tel quel.

### Hypatia — matrice de tests
Conclusion :
- le point le plus important etait un test avec `media.json` preexistant et skippé ;
- il fallait aussi verrouiller l'incompatible MIME en overwrite avec asset local ;
- un test cry miroir restait utile et peu couteux.

Decision retenue :
- retenu ;
- ajout des tests `skipExisting` / image incompatible / image incompatible en overwrite / cry incompatible.

### Mendel — revue stricte de scope
Conclusion :
- le fix pouvait rester strictement dans le use case et son fichier de test ;
- toucher aux providers/UI/ports aurait ete hors scope.

Decision retenue :
- retenu ;
- aucun autre fichier code hors cible n'a ete modifie.

### Avicenna — audit preuve report / git
Conclusion :
- pour un report non tracke, les preuves git pertinentes sont `git status --short` et `git ls-files --others --exclude-standard` ;
- le report n'existait pas avant ce mini-fix.

Decision retenue :
- retenu ;
- preuves git finales explicites incluses ci-dessous.

## 6. Design retenu

Design minimal retenu :
- conserver l'orchestration actuelle ;
- inserer une garde juste avant `_downloadBestEffortAssets(...)` ;
- si `mediaPlan.action == PokemonExternalImportArtifactAction.skip`, retourner un batch vide avec warning global ;
- ne pas reecrire `media.json` ;
- ne pas creer de pseudo-resultats asset par asset qui laisseraient croire qu'un download a ete tente.

Pourquoi ce design :
- il corrige le bug a la racine ;
- il ne change pas la semantique de `species`, `learnset` ou `evolution` ;
- il n'introduit aucun nouveau service ;
- il garde intact le traitement deja correct des `content-type` incompatibles.

## 7. Liste exacte des fichiers touches
...
### `packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart`
- ajout de la garde qui bloque tout nouveau download media si `media.json` est en `skip` ;
- ajout d'un helper local `_buildSkippedMediaAssetBatch(...)` ;
- commentaires renforces sur l'invariant anti-assets-orphelins.

### `packages/map_editor/test/import_external_pokemon_use_cases_test.dart`
- ajout d'un test de reproduction du bug `skipExisting` + `media.json` skip ;
- ajout de tests de verrouillage sur les `content-type` incompatibles pour image et cry ;
- les anciens tests existants sont conserves.

### `reports/phase-11a-mini-fix-2-report.md`
- report demande par le prompt ;
- preuves git reelles ;
- annexe contenant le contenu complet des autres fichiers texte touches.
...
git status --short -- packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart packages/map_editor/test/import_external_pokemon_use_cases_test.dart reports/phase-11a-mini-fix-2-report.md
sed -n '1,260p' packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart
sed -n '1,320p' packages/map_editor/test/import_external_pokemon_use_cases_test.dart
rg -n "skipExisting|content-type|contentType|media artifact|downloadedAssets|_downloadBestEffortAssets|_resolvePersistedMediaFromDisk|GIF|incompatible" packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart packages/map_editor/test/import_external_pokemon_use_cases_test.dart
sed -n '320,760p' packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart
sed -n '760,1280p' packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart
sed -n '320,760p' packages/map_editor/test/import_external_pokemon_use_cases_test.dart
```
...
```bash
git status --short -- packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart packages/map_editor/test/import_external_pokemon_use_cases_test.dart reports/phase-11a-mini-fix-2-report.md
git diff --stat -- packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart packages/map_editor/test/import_external_pokemon_use_cases_test.dart reports/phase-11a-mini-fix-2-report.md
git ls-files --others --exclude-standard -- reports/phase-11a-mini-fix-2-report.md
sed -n '1,40p' reports/phase-11a-mini-fix-2-report.md
```
...
### Reproduction avant correction
Commande : `flutter test test/import_external_pokemon_use_cases_test.dart`

Resultat cle avant patch :
```text
ImportExternalPokemonSpeciesUseCase skip_existing does not download new assets when media.json is already skipped [E]
Expected: <0>
  Actual: <6>
```

Observation honnete :
- ce run a prouve le bug `skipExisting` ;
- les nouveaux tests `content-type` n'ont pas echoue dans ce meme run, ce qui confirmait deja que leur comportement etait correct.

### Validation apres correction
`dart format ...`
```text
Formatted packages/map_editor/test/import_external_pokemon_use_cases_test.dart
...
- un `apply_patch` a time out pendant l'edition du use case ;
- le fichier a ete relu immediatement pour verifier qu'aucun demi-patch n'avait ete applique ;
- le patch a ensuite ete rejoue en deux morceaux plus petits.

Aucun autre incident bloquant.

## 12. Etat git utile final
...
`git diff --stat -- packages/map_editor/lib/src/application/use_cases/import_external_pokemon_use_cases.dart packages/map_editor/test/import_external_pokemon_use_cases_test.dart reports/phase-11a-mini-fix-2-report.md`
```text
 .../import_external_pokemon_use_cases.dart         |  55 +++++-
 .../import_external_pokemon_use_cases_test.dart    | 189 +++++++++++++++++++++
...
- ce mini-fix ne nettoie pas retroactivement d'anciens assets orphelins crees avant lui ;
- il ne tente pas de migrer d'anciens `media.json` deja presents ;
- il reste volontairement limite au use case d'import externe unitaire ;
- il ne modifie ni le batch produit, ni l'UI, ni les providers.

## 14. Checklist finale

- [x] mini-fix strictement limite au scope
- [x] pas de nouveau port
- [x] pas de nouveau use case parallele
- [x] pas de changement `project.json`
- [x] pas de phase 11B
- [x] cas `skipExisting` media audite et corrige
- [x] cas `content-type` incompatible couvert par tests
- [x] aucun `media.json` nouvellement ecrit ne reference un asset absent
- [x] cry coherent
- [x] animations coherentes
- [x] warnings honnetes
- [x] analyze passe
- [x] tests passent
- [x] report cree
- [x] report prouve par git
- [x] report contient l'integralite des fichiers texte touches, sauf lui-meme pour eviter une recursion infinie
- [x] code manuel massivement commente

## 15. Contenu complet de TOUS les fichiers texte modifies/crees

Note explicite :
- le report present n'est pas recopie integralement dans lui-meme pour eviter une recursion infinie ;
- tous les autres fichiers texte modifies pendant ce mini-fix sont reproduits integralement ci-dessous.
