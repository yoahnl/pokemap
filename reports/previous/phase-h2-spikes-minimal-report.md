# Phase H2 — Spikes Minimal Layered Slice Report

## 1. Résumé exécutif honnête
Verdict : **H2 est réussi**.

Ce lot implémente uniquement `Spikes`, en restant strictement dans le slice demandé :
- pose sur le side adverse ;
- 0 à 3 couches ;
- non-stackabilité au-delà de 3 ;
- dégâts à l’entrée avec règle locale de groundedness strictement bornée ;
- coexistence déterministe avec H1 `Stealth Rock` (`Stealth Rock` puis `Spikes`) ;
- déclenchement honnête sur switch volontaire, auto-switch ennemi et remplacement forcé joueur ;
- timeline observable et bridge runtime honnête ;
- seed bootstrap réaligné uniquement pour `spikes`.

Ce lot **n’ouvre pas** Toxic Spikes, Sticky Web, retrait de hazards, abilities, items, groundedness générale, framework générique de hazards, framework générique de side conditions, selfSwitch, forceSwitch, targeting riche ou doubles.

Confirmé par exécution :
- `packages/map_battle` analyze vert ;
- `packages/map_battle` tests complets verts ;
- runtime analyze ciblé vert ;
- runtime tests ciblés + smoke Phase A verts ;
- map_editor analyze/test seed ciblés verts.

Confirmé par lecture de code :
- `BattleSideState.spikesLayers` est vivant et consommé ;
- `BattleSpikesEvent` est un contrat dédié et borné ;
- `_resolveEntryHazards()` impose explicitement l’ordre `Stealth Rock` puis `Spikes` ;
- le bridge runtime accepte `spikes` et refuse toujours les autres side conditions hors H1/H2.

Point important : le reviewer séparé a trouvé un vrai problème de champ mort (`sourceMoveId`) dans `BattleSpikesEvent`. Il a été retiré, puis la validation a été relancée proprement.

## 2. Verdict global
- Décision finale : **H2 réussi**.
- Nature du lot : **vrai lot H2 borné**, pas une généralisation de H1/H2 en framework de hazards.
- Blast radius : limité à `map_battle`, runtime bridge/overlay strictement utiles, et seed bootstrap minimal pour la vérité produit.
- Effet architectural réel : deuxième état side-level vivant après H1, avec couche(s) entières explicitement stockées et réellement consommées par le moteur.

## 3. Pré-gates et état git initial
Pré-gates exécutés au début du lot :
- `git status --short --untracked-files=all`
- `git diff --stat`
- `git ls-files --others --exclude-standard`

Résultat initial observé : worktree propre, aucun changement local avant H2. Cela a permis d’isoler honnêtement le blast radius du lot.

Réexécution finale read-only :
```text
 ...
 M packages/map_battle/lib/src/battle_stealth_rock.dart
...
```text
 ...
 packages/map_battle/lib/src/battle_queue.dart      |   4 +-
 packages/map_battle/lib/src/battle_resolution.dart |  19 ++-
 packages/map_battle/lib/src/battle_session.dart    | 185 +++++++++++++++++++--
 packages/map_battle/lib/src/battle_setup.dart      |  10 ++
 packages/map_battle/lib/src/battle_state.dart      |  29 ++++
 .../map_battle/lib/src/battle_stealth_rock.dart    |   3 +-
 .../seeds/pokemon_moves_bootstrap_seed.dart        |  35 +++-
 .../test/pokemon_moves_bootstrap_seed_test.dart    |  29 +++-
 .../application/runtime_battle_move_bridge.dart    |  38 +++--
 .../flame/battle_overlay_component.dart            |  17 ++
 .../test/battle_overlay_component_test.dart        |  89 ++++++++++
 .../test/runtime_battle_move_bridge_test.dart      |  40 ++++-
...
Ordre réel suivi :
1. lecture des reports Phase A à H1 et du `AGENTS.md` racine ;
2. audit battle-core réel sur les seams H1/H2 ;
3. audit scope creep via sub-agent dédié ;
4. TDD ciblé H2 sur les cas structurants ;
5. implémentation minimale battle/runtime/seed ;
6. validations ciblées puis plus larges ;
7. review séparée réelle ;
8. correction du finding reviewer ;
9. reruns complets post-review ;
10. rédaction du report final ultra-complet.

Le lot a été traité comme un slice parallèle à H1, pas comme le point de départ d’un système générique de side conditions.

## 5. Audit réel avant code
### 5.1. Ce que le code réel montrait déjà
Confirmé par lecture de code :
- H1 avait déjà ouvert un seam side-level vivant avec `hasStealthRock` sur `BattleSideState`.
- Phase D donnait déjà une topologie side/slot honnête.
- Phase F donnait déjà une queue et des seams de suspension/reprise de tour honnêtes pour les cas où un entrant meurt pendant une chaîne de résolution.
- `BattleSession` possédait déjà un moment explicite où les entry hazards d’H1 étaient appliqués sur switch volontaire, auto-switch ennemi et remplacement forcé joueur.
- Le bridge runtime savait déjà transporter un move side-level ciblant `BattleMoveTarget.opponentSide` pour `Stealth Rock`.

### 5.2. Les vrais blockers H2
`required_now` :
- un vrai état side-level vivant pour des couches `0..3` ;
- un payload move explicitement capable de dire “ce move pose Spikes” ;
- une famille d’événements observable dédiée pour la pose, le cas max et les dégâts d’entrée ;
- un seam de résolution d’entrée explicitement composé avec H1 dans un ordre déterministe ;
- un bridge runtime honnête pour `spikes` ;
- un réalignement bootstrap minimal si `spikes` devient réellement supporté de bout en bout.

`not_required_yet` :
- framework générique de side conditions ;
- framework générique de hazards ;
- groundedness réutilisable pour items/abilities ;
- retrait de hazards ;
- side conditions autres que H1/H2 ;
- migration Phase E/F supplémentaire.

`belongs_to_H_not_G` ou hors H2 :
- Toxic Spikes ;
- Sticky Web ;
- Levitate / Air Balloon / Magnet Rise / Gravity ;
- Heavy-Duty Boots ;
- Defog / Rapid Spin / Mortal Spin / Tidy Up ;
- generic hazard registry.

### 5.3. Faux progrès explicitement écartés
- remplacer `hasStealthRock` par une `Map<SideConditionId, dynamic>` ;
- créer un `BattleHazardRegistry` ;
- réutiliser `BattleConditionEngine` comme faux moteur de side conditions ;
- créer un helper de groundedness “pour plus tard” ;
- réécrire la queue pour en faire un scheduler d’interruptions plus large.

Conclusion d’audit : **H2 Spikes est légitime maintenant**, mais uniquement comme slice dédié et non générique.

## 6. Critique explicite du prompt
Le prompt était globalement bon :
- il protégeait bien le scope ;
- il exigeait la bonne honnêteté produit ;
- il bloquait explicitement le piège du “framework de hazards”.

Points discutables ou à recadrer :
1. Le prompt pousse fort vers un nouveau fichier dédié et une famille d’événements dédiée. Ici c’était la bonne décision, mais ce type d’exigence est normalement une conséquence de l’audit, pas une vérité a priori.
2. Le prompt fixe une définition locale de groundedness (`Flying => non grounded`, sinon grounded, `typing == null => grounded`). C’est volontairement non-Showdown et donc **pas une vérité battle générale**. Il fallait le traiter comme une règle H2 strictement locale, pas comme un début de système général.
3. Il existe une tension entre “ne touche pas `map_editor`” et “ne mens pas sur le bootstrap si `spikes` devient réellement supporté”. La plus petite option honnête était de toucher exactement une entrée seed et un test seed, pas davantage.
4. La partie runtime/UI pouvait sembler sur-prescriptive. En pratique, un tout petit changement overlay était effectivement justifié pour garder la timeline honnête et observable.

Aucune entorse de scope n’a été prise sans justification explicite.

## 7. Périmètre inclus / exclu
### Inclus
- `packages/map_battle/**` sur la surface strictement nécessaire à H2 ;
- runtime bridge minimal pour `spikes` ;
- overlay timeline minimal pour afficher honnêtement les nouveaux événements ;
- seed bootstrap minimal pour la vérité produit ;
- tests battle/runtime/seed utiles.

### Exclu volontairement
- `examples/**`
- `packages/map_core/**`
- toute mécanique autre que `Spikes`
- toute généralisation hazard/side-condition
- retrait de hazards
- Toxic Spikes / Sticky Web
- groundedness générale
- abilities/items
- doubles
- refonte Phase E/F
- refonte UI/runtime au-delà de la consommation honnête de la timeline.

### Exception justifiée
`packages/map_editor` a été touché uniquement pour l’entrée seed `spikes`, car laisser le bootstrap mentir après support réel aurait contredit la doctrine canonique.

## 8. Design retenu
Design final retenu, volontairement petit :
- `BattleSideState.spikesLayers` stocke un entier `0..3` ;
- `BattleMove.setsSpikes` / `BattleMoveData.setsSpikes` portent explicitement le payload move H2 ;
- `battle_spikes.dart` contient :
  - `BattleSpikesEventKind`
  - `BattleSpikesEvent`
  - `isGroundedForSpikesEntry()`
  - `resolveSpikesEntryDamage()`
- `BattleTurnResult` transporte `spikesEvents` ;
- `BattleSession` résout :
  - la pose des couches ;
  - le cas déjà max ;
  - les dégâts d’entrée ;
  - l’ordre explicite `Stealth Rock` puis `Spikes` via `_resolveEntryHazards()`.

Pourquoi ce design est bon pour H2 :
- chaque ajout a un usage immédiat ;
- aucun conteneur générique mort n’est introduit ;
- l’ordre d’entrée n’est pas implicite ;
- le runtime n’a besoin que d’un bridge side-condition minimal et d’un affichage overlay minimal.

Pourquoi ce design refuse de généraliser :
- `spikesLayers` est spécifique et vivant ;
- `BattleSpikesEvent` est spécifique et vivant ;
- groundedness reste un helper local à H2 ;
- aucune map de side conditions n’existe ;
- aucun registry ou système de callbacks n’existe.

## 9. Justification fichier par fichier
- `packages/map_battle/lib/map_battle.dart` : exporte `battle_spikes.dart` pour rendre le contrat H2 consommable.
- `packages/map_battle/lib/src/battle_move.dart` : ajoute `setsSpikes` au contrat move local, immédiatement consommé par la session.
- `packages/map_battle/lib/src/battle_queue.dart` : commentaire de frontière mis à jour pour mentionner H1/H2 ; pas de refonte de queue.
- `packages/map_battle/lib/src/battle_resolution.dart` : ajoute `spikesEvents` et `BattleTurnSpikesEvent` pour l’observabilité honnête.
- `packages/map_battle/lib/src/battle_session.dart` : cœur du lot H2 ; pose, dégâts d’entrée, ordre SR puis Spikes, suspension/reprise honnête.
- `packages/map_battle/lib/src/battle_setup.dart` : transporte `setsSpikes` au setup local.
- `packages/map_battle/lib/src/battle_state.dart` : ajoute `spikesLayers` à `BattleSideState` avec bornes 0..3.
- `packages/map_battle/lib/src/battle_stealth_rock.dart` : commentaire de frontière ajusté pour coexister explicitement avec H2.
- `packages/map_battle/lib/src/battle_spikes.dart` : nouveau fichier dédié H2, petit et vivant.
- `packages/map_battle/test/battle_spikes_test.dart` : nouveau fichier de tests H2 structurants.
- `packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart` : support honnête de `spikes`, rejet explicite des autres side conditions.
- `packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart` : restitution honnête des événements Spikes dans la timeline.
- `packages/map_runtime/test/battle_overlay_component_test.dart` : verrouille l’affichage overlay H2.
- `packages/map_runtime/test/runtime_battle_move_bridge_test.dart` : verrouille support `spikes` et rejet des autres side conditions.
- `packages/map_editor/lib/src/application/seeds/pokemon_moves_bootstrap_seed.dart` : réaligne uniquement l’entrée `spikes` dans le seed embarqué.
- `packages/map_editor/test/pokemon_moves_bootstrap_seed_test.dart` : verrouille la vérité bootstrap H2.

## 10. Classification des blockers réellement adressés
| Élément | Classification | Décision |
|---|---|---|
| `BattleSideState.spikesLayers` | `required_now` | Ajouté, vivant et borné à 0..3 |
| `BattleMove.setsSpikes` | `required_now` | Ajouté, consommé en prod |
| `BattleMoveData.setsSpikes` | `required_now` | Ajouté, consommé par le bridge/setup |
| `BattleSpikesEvent` | `required_now` | Ajouté, observé par timeline/overlay/tests |
| Ordre `Stealth Rock` puis `Spikes` | `required_now` | Implémenté explicitement |
| Groundedness locale H2 | `required_now` | Ajoutée localement dans `battle_spikes.dart` |
| Side condition registry générique | `rejected_dead_field` | Refusé |
| Generic hazard framework | `rejected_dead_field` | Refusé |
| Generic groundedness system | `not_required_yet` | Refusé pour H2 |
| Toxic Spikes / Sticky Web | `belongs_to_H_not_G` / hors H2 | Refusé |
| Hazard removal | `belongs_to_H_not_G` / hors H2 | Refusé |
| Items / abilities / Flying immunities étendues | `belongs_to_H_not_G` | Refusé |
| `BattleFieldState` expansion | `not_required_yet` | Inchangé |
| `BattleVolatileState` expansion | `not_required_yet` | Inchangé |
| `BattleConditionEngine` extension | `not_required_yet` | Inchangé |


## 11. Commandes réellement exécutées
Commandes significatives réellement lancées pendant ce lot :
...
Validation et TDD intermédiaires :
- `dart format packages/map_battle/lib/map_battle.dart packages/map_battle/lib/src/battle_move.dart packages/map_battle/lib/src/battle_queue.dart packages/map_battle/lib/src/battle_resolution.dart packages/map_battle/lib/src/battle_session.dart packages/map_battle/lib/src/battle_setup.dart packages/map_battle/lib/src/battle_state.dart packages/map_battle/lib/src/battle_stealth_rock.dart packages/map_battle/lib/src/battle_spikes.dart packages/map_battle/test/battle_spikes_test.dart packages/map_runtime/lib/src/application/runtime_battle_move_bridge.dart packages/map_runtime/lib/src/presentation/flame/battle_overlay_component.dart packages/map_runtime/test/battle_overlay_component_test.dart packages/map_runtime/test/runtime_battle_move_bridge_test.dart packages/map_editor/lib/src/application/seeds/pokemon_moves_bootstrap_seed.dart packages/map_editor/test/pokemon_moves_bootstrap_seed_test.dart`
- `cd packages/map_battle && dart test test/battle_spikes_test.dart`
...
- `cd packages/map_runtime && flutter analyze --no-pub lib/src/application/runtime_battle_move_bridge.dart lib/src/presentation/flame/battle_overlay_component.dart test/runtime_battle_move_bridge_test.dart test/battle_overlay_component_test.dart test/runtime_battle_setup_mapper_test.dart test/phase_a_golden_battle_slice_smoke_test.dart`
...
Review coordination :
- message envoyé au reviewer séparé avec follow-up après correction ;
- premier `wait` expiré sans réponse exploitable ;
- réponse finale reçue ensuite : aucun finding concret restant, plus un rerun reviewer de :
  - `dart test test/battle_spikes_test.dart`
  - `flutter test test/battle_overlay_component_test.dart test/runtime_battle_move_bridge_test.dart`

## 12. Résultats réels format / analyze / tests / smoke
Confirmé par exécution :
- format ciblé : vert (`0 changed` au dernier rerun) ;
- `packages/map_battle`: `dart analyze lib test` vert ;
- `packages/map_battle`: `dart test` complet vert ;
- `packages/map_battle`: `dart test test/battle_spikes_test.dart` vert ;
- `packages/map_runtime`: analyze ciblé vert ;
- `packages/map_runtime`: `flutter test test/runtime_battle_move_bridge_test.dart test/battle_overlay_component_test.dart test/runtime_battle_setup_mapper_test.dart test/phase_a_golden_battle_slice_smoke_test.dart` vert ;
- smoke Phase A golden slice : vert (via le test runtime ciblé) ;
- `packages/map_editor`: analyze ciblé vert ;
- `packages/map_editor`: test seed ciblé vert.

Aucune validation finale n’est restée “supposée”.

## 13. Incidents rencontrés
1. **Bridge runtime initialement trop tôt sur `foeSide`**
   - Symptôme : un test runtime montrait qu’un move comme `toxic_spikes` était rejeté en amont comme `unsupported_target:foeSide` au lieu d’atteindre le rejet honnête `unsupported_side_condition:<id>`.
   - Correction : j’ai élargi le helper de forme pour reconnaître les candidats “pure foe-side side condition” sans pour autant les supporter, afin qu’ils tombent ensuite dans le rejet explicite des side conditions non ouvertes.

2. **Champ mort reviewer-found (`sourceMoveId`)**
   - Symptôme : le reviewer a pointé que `BattleSpikesEvent.sourceMoveId` n’avait aucun consommateur prod réel.
   - Correction : le champ a été supprimé.

3. **Nettoyage reviewer incomplet au premier passage**
   - Symptôme : après retrait du champ, le constructeur `BattleSpikesEvent.damagedOnEntry` gardait encore un initializer `sourceMoveId = null`, ce qui a cassé `dart analyze` et `dart test`.
   - Correction : initializer supprimé, puis reruns complets.

4. **Follow-up reviewer tardif**
   - Le premier `wait` a expiré, donc la clôture technique a d’abord reposé sur le finding initial corrigé + les reruns complets post-fix.
   - Une réponse reviewer finale est ensuite arrivée : aucun finding concret restant, avec en plus un mini-rerun ciblé battle/runtime.

## 14. Décisions retenues / rejetées
### Retenues
- état side-level concret `spikesLayers` ;
- move payload concret `setsSpikes` ;
- événements dédiés `BattleSpikesEvent` ;
- helper local `isGroundedForSpikesEntry()` ;
- helper local `resolveSpikesEntryDamage()` ;
- composition explicite `_resolveEntryHazards()` avec ordre `Stealth Rock` puis `Spikes` ;
- bridge runtime minimal pour `spikes` uniquement ;
- overlay timeline minimal pour la restitution honnête.

### Rejetées
- side condition registry générique ;
- hazard framework ;
- groundedness générique ;
- side condition enum/map multi-usages ;
- extension à Toxic Spikes / Sticky Web ;
- retrait de hazards ;
- abilities/items ;
- refonte queue/condition engine ;
- refonte bootstrap hors `spikes`.


## 15. Retour des sub-agents
### Sub-agent audit/design battle-core : Aristotle
Synthèse utile retenue :
- H2 est légitime uniquement comme slice parallèle à H1, pas comme début de “hazard system” ;
- le bon seam vivant est `spikesLayers` dans `BattleSideState` ;
- il faut un contrat observable dédié et un orchestrateur explicite d’entrée ;
- il faut réutiliser la topologie side/slot de D et les seams de timing H1/F existants.

Ce que j’ai retenu : oui.
Ce que j’ai rejeté : rien d’important ; ses recommandations allaient dans le bon sens du prompt.

### Sub-agent scope creep : Copernicus
Synthèse utile retenue :
- refuser tout `BattleSideConditionsState` générique ;
- refuser tout `hazard registry` ;
- refuser groundedness générale ;
- refuser toute ouverture simultanée de Toxic Spikes / Sticky Web / retrait ;
- n’accepter qu’un bridge minimal `spikes` côté runtime.

Ce que j’ai retenu : oui, presque intégralement.
Ce que j’ai rejeté : aucune extension de scope proposée n’a été acceptée.


## 16. Retour du reviewer séparé
Reviewer final : **Archimedes**.

Finding concret retenu :
- `BattleSpikesEvent.sourceMoveId` élargissait le contrat H2 sans consommateur réel. C’était un vrai champ mort, contraire à la règle canonique.

Points explicitement challengés par le reviewer :
- risque de faux framework de side conditions ;
- risque de champ mort dans les événements H2 ;
- risque de mismatch entre topologie side/slot et charge observable ;
- risque de blast radius runtime inutile.

Le reviewer n’a pas remonté d’autre finding concret exploitable dans son retour initial.

Follow-up final reçu après correction :
- aucun finding concret restant ;
- reruns reviewer annoncés :
  - `dart test test/battle_spikes_test.dart`
  - `flutter test test/battle_overlay_component_test.dart test/runtime_battle_move_bridge_test.dart`

## 17. Corrections appliquées après review
Corrections effectivement appliquées :
1. suppression de `sourceMoveId` dans `BattleSpikesEvent` ;
2. suppression de son initializer orphelin dans `BattleSpikesEvent.damagedOnEntry` ;
3. consommation réelle de `targetSlot.side` dans l’overlay pour les dégâts d’entrée, afin que `targetSlot` reste un champ vivant et pas décoratif ;
4. reruns complets battle/runtime/map_editor après correction.


## 18. Autocritique finale
Le lot est solide, mais il reste volontairement étroit.

Limites assumées :
- la groundedness H2 est volontairement simplifiée et non réutilisable hors H2 ;
- aucun retrait de hazards n’existe ;
- aucune immunité item/ability n’existe ;
- la coexistence d’entrée ne gère explicitement que H1 + H2 ;
- le seed bootstrap a été touché par nécessité produit, ce qui reste un petit écart au “battle-only” théorique.

Ce que je referais pareil :
- garder un fichier dédié `battle_spikes.dart` plutôt qu’un faux framework ;
- garder `spikesLayers` plutôt qu’un conteneur générique ;
- garder un seam d’ordre d’entrée explicite.

Ce que je surveillerais au lot suivant :
- ne pas généraliser trop tôt H1+H2 en hazard framework ;
- ne pas laisser une future mécanique réutiliser abusivement la groundedness H2 comme si elle était générale.
...
## 20. Checklist finale
- [x] ai-je implémenté uniquement Spikes ?
- [x] ai-je évité toute généralisation morte ?
- [x] `spikesLayers` est-il vivant et réellement consommé ?
- [x] la pose 1/2/3 couches est-elle honnête ?
- [x] le cas déjà max est-il honnête ?
- [x] les dégâts d’entrée sont-ils honnêtes ?
- [x] la règle locale de groundedness est-elle explicite et bornée ?
- [x] la coexistence avec H1 Stealth Rock est-elle honnête ?
- [x] les timings de switch / auto-switch / replacement sont-ils honnêtes ?
- [x] ai-je évité Toxic Spikes / Sticky Web / retrait / items / abilities ?
- [x] ai-je exécuté analyze/tests/smoke utiles ?
- [x] ai-je fait une vraie review séparée ?
- [x] ai-je intégré les remarques valides ?
- [x] ai-je inclus tous les fichiers touchés dans le report ?
- [x] ai-je évité toute écriture Git interdite ?


## 21. Décision finale : H2 réussi ou non
**H2 est réussi.**

Le lot est un vrai H2 :
- il introduit une side condition side-level vivante propre à `Spikes` ;
- il ajoute des couches vivantes et un déclenchement d’entrée réel ;
- il garde l’ordre d’entrée honnête avec H1 ;
- il reste observé en timeline et consommé par le runtime ;
- il ne s’étend pas vers un système générique de hazards.

## 22. Ce que ce lot débloque exactement pour la suite
Ce que H2 débloque réellement :
- un deuxième slice side-level riche qui prouve que `BattleSideState` peut porter autre chose qu’un booléen H1 ;
- une coexistence d’entrée déterministe entre deux hazards spécifiques ;
- un pattern honnête pour ajouter d’autres lots H **seulement** s’ils restent aussi bornés ;
- une meilleure base pour un futur lot ciblé de side condition riche, sans forcer de framework maintenant.

Ce que H2 **ne** débloque toujours pas :
- Toxic Spikes ;
- Sticky Web ;
- retrait de hazards ;
- items/abilities liés aux hazards ;
- groundedness générale ;
- selfSwitch / forceSwitch ;
- doubles ;
- système générique de side conditions.

Prochain lot logique seulement si explicitement justifié :
- pas de prochain lot “obligatoire” imposé par H2 ;
- si la trajectoire reste sur des side conditions riches ciblées, un **H3 isolé** pourrait viser une mécanique unique supplémentaire, mais il faudrait rouvrir proprement ses règles produit propres au lieu de réutiliser naïvement les simplifications H2.

## 23. Contrats changés / non changés et pourquoi
| Contrat | Statut | Pourquoi | Lien avec G/H |
|---|---|---|---|
| `BattleMove` | changé | `setsSpikes` requis maintenant et consommé par la prod | H2 requis, pas décoratif |
| `BattleSetup` | changé | transporte `setsSpikes` honnêtement | H2 requis |
| `BattleSideState` | changé | `spikesLayers` est le vrai état side-level vivant du lot | H2 requis |
| `BattleTurnResult` / `battle_resolution.dart` | changé | observabilité honnête H2 | H2 requis |
| `BattleSession` | changé | logique réelle du lot | H2 requis |
| `BattleConditionEngine` | non changé | H2 ne nécessite pas de migration du condition engine | pas requis maintenant |
| `BattleQueue` | non changé contractuellement | le seam F suffit déjà ; seulement commentaire de frontière ajusté | pas requis maintenant |
| `BattleFieldState` | non changé | H2 ne vit pas au niveau field | non requis |
| `BattleVolatileState` | non changé | H2 ne vit pas au niveau volatile | non requis |
| Runtime bridge | changé | support honnête `spikes` + rejet des autres side conditions | H2 requis |
| Overlay runtime | changé | restitution honnête de la timeline H2 | H2 requis |
| Seed bootstrap | changé minimalement | vérité produit obligatoire après support réel | exception justifiée |


## 24. Contenu complet de tous les fichiers modifiés / créés / supprimés
Le report exclut volontairement son propre contenu pour éviter une récursion absurde. Tous les autres fichiers touchés par le lot sont inclus intégralement ci-dessous.
