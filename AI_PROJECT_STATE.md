...
### Couche opérations (19 modules)

Fonctions pures pour modifier les données en éditeur : `addEntityOnMap`, `removeEntityFromMap`, `updateEntityOnMap`, `resizeMap`, `paintTile`, `toggleCollision`, `addWarp`, `updateMapMetadata`, `upsertMapPlacedElement`, `setMapPlacedElementCollisionApplied`, et désormais `map_events` (`add/update/move/remove event`, `add/update/remove page`). Toutes retournent `MapData` modifié (immutable).
...
- Authoring d’events dans l’éditeur : outil dédié, pose/sélection/suppression sur canvas, marqueurs visuels d’event, inspecteur `Map Events` pour éditer `id`/`title`/`type`/`position`/`pages`, et édition de page (`message`, `scriptId`, `startNode`, condition MVP).
