...
void main() {
  test(
    'application bootstrap uses a platform-neutral Hub composition',
    () async {
      final main = await File('lib/main.dart').readAsString();
      final bootstrap = await File('lib/app/app_root.dart').readAsString();
      final composition =
      ...
      expect(
        bootstrap,
        contains(
          'package:pokemap_hub/core/config/public_product_identity.dart',
        ),
      );
      expect(
        composition,
  ...
  test(
    'iOS adapter uses the native Hub channel for picker and disk space',
    () async {
      final adapter =
          await File(
  ...
  test(
    'iOS runner exposes only the native .avelunegame document picker',
    () async {
      final appDelegate =
      ...
      expect(appDelegate, contains('let packageTypes = ["avelunegame"].map'));
      expect(appDelegate, isNot(contains('pokemapgame')));
      expect(appDelegate, contains('case "pickPackage"'));
      expect(appDelegate, contains('case "availableDiskBytes"'));
      expect(appDelegate, contains('volumeAvailableCapacityForImportantUsage'));
      ...
      expect(composition, contains("<String>{'.avelunegame'}"));
      expect(composition, isNot(contains('.pokemapgame')));
