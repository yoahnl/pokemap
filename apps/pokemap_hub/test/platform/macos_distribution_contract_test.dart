...
void main() {
  test('macOS runner declares the generic Hub and .avelunegame type', () async {
    ...
    expect(info, contains('<string>application/x-avelune-game</string>'));
    expect(info, contains('<string>Avelune Game Package</string>'));
    expect(info, isNot(contains('pokemapgame')));
  });

  test(
    'Release enables hardened runtime with least-privilege entitlements',
    () async {
      final project =
  ...
  test(
    'macOS reports a missing picker entitlement instead of failing silently',
    () async {
      final window =
      ...
      expect(adapter, contains("invokeMethod<bool>('canSelectPackages')"));
  ...
  test('macOS package picker uses the Hub native bridge', () async {
    ...
    expect(window, contains('UTType(filenameExtension: "avelunegame")'));
    expect(window, isNot(contains('pokemapgame')));
    expect(adapter, contains("invokeMethod<String>('pickPackage')"));
    expect(adapter, isNot(contains('openFile(')));
  });

  test(
    'application entry point composes PokeMap Hub, not a demo counter',
    () async {
  ...
  test(
    'macOS open-file events are forwarded to the Dart import bridge',
    () async {
      final appDelegate =
      ...
      expect(
        adapter,
        contains("MethodChannel('app.pokemap.hub/package_open')"),
      );
